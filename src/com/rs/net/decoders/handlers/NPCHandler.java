...
public class NPCHandler {
	...
	public static void handleOption1(final Player player, final InputStream stream) {
		...
		if(Settings.DEBUG) {
			if(player.getRights() == 2) {
				player.sm("DEBUG: NPC ID "+npc.getId());
			}
		}
		if(npc.getId() == 4288) {
		...
		if(npc.getId() == 8269) {
			player.getDialogueManager().startDialogue("SloaneDialouge", npc);
		}
		if(npc.getId() == 682) {
			player.getDialogueManager().startDialogue("ArmourSalesman", npc);
		}
		if(npc.getId() == 802) {
			player.getDialogueManager().startDialogue("BrotherJered", npc);
		}
		if(npc.getId() == 13632) {
			player.getDialogueManager().startDialogue("Larriar", npc);
		}
		if (npc.getId() == 733) {
			npc.resetWalkSteps();
			npc.faceEntity(player);
			...
			player.getDialogueManager().startDialogue("PaulinePolaris", npc);
		}
		...
		if(npc.getId() == 2438) {
			player.setNextFaceWorldTile(new WorldTile(2620, 3685, 0));
		}
		if(npc.getId() == 2437) {
			player.setNextFaceWorldTile(new WorldTile(2551, 3759, 0));
		}
		...
		player.setRouteEvent(new RouteEvent(npc, new Runnable() {
			...
			public void run() {
				...
				if (player.getTreasureTrailsManager().useNPC(npc))
					return;
				else if (npc instanceof GraveStoneHandler) {
				    GraveStoneHandler gsh = (GraveStoneHandler) npc;
				...
				/*else if (npc.getName().contains("Wise Old Man"))
				...
				if (player.getTreasureTrailsManager().useNPC(npc))
					...
					player.getDialogueManager().startDialogue("SurgeonGeneralTafani", npc);
					...
					OldMagicSystem.sendJewerlyTeleportSpell(player, true, 9603, 1684, 4, new WorldTile(3040, 4845, 0));
		...
		player.setRouteEvent(new RouteEvent(npc, new Runnable() {
			...
			public void run() {
				...
				if (npc instanceof Familiar) {
					...
					GraveStoneHandler grave = (GraveStoneHandler) npc;
				    grave.repairGravestone(player, false);
				    return;
				}
				 ...
				 * 
				 * 
				 * EXEMPLO else if(npc.getId() == id_npc)
				 * 				ShopsHandler.openShop(player, numero_da_loja);
				 */
				...
				if(npc.getId() == 534)
					...
					player.getDialogueManager().startDialogue("tanHide", npc.getId());
				} else if (npc.getId() == 3373) {
	...
	public static void handleOption3(final Player player, InputStream stream) {
		...
		if (npc.getDefinitions().name.toLowerCase().contains("banker")
		    ...
		    GraveStoneHandler grave = (GraveStoneHandler) npc;
		    grave.repairGravestone(player, true);
		    return;
		}
		if(npc.getId() == 209) {
			if(player.getLostCannon()) {
				if(player.getInventory().getFreeSlots() >= 4) {
					player.getInventory().addItem(new Item(6, 1));
					player.getInventory().addItem(new Item(8, 1));
					player.getInventory().addItem(new Item(10, 1));
					player.getInventory().addItem(new Item(12, 1));
					player.lostCannon(false);
					return;
				}else {
					player.sm("You have no space in the inventory.");
					return;
				}
			}
				
				player.sm("You have no cannon to recover");
				return;
		}
		if (npc.getId() == 2593 || npc.getDefinitions().name.toLowerCase().contains("clerk")) {
			player.setRouteEvent(new RouteEvent(npc, new Runnable() {
		...
		player.setRouteEvent(new RouteEvent(npc, new Runnable() {
			...
			public void run() {
				...
				if (SlayerMaster.startInteractionForId(player, npc.getId(), 3))
					...
					player.getDialogueManager().startDialogue("EncantDialogue");
				else if (npc.getId() == 548)
					PlayerLook.openThessaliasMakeOver(player);
				else if (npc.getId() == 1301)
					PlayerLook.openYrsaShop(player);
				else if (npc.getId() == 3373) {
					player.getCustomStore().sendInterface(player, 3, 0);
					return;
				} else if (npc.getId() == 8929) {
	...
	public static void handleOption4(final Player player, InputStream stream) {
		...
		if (forceRun)
			player.setRun(forceRun);
		if (npc instanceof GraveStoneHandler) {
			GraveStoneHandler grave = (GraveStoneHandler) npc;
		...
		player.setRouteEvent(new RouteEvent(npc, new Runnable() {
			...
			public void run() {
				...
				if ((npc.getId() == 8462 || npc.getId() == 8464 || npc.getId() == 1597 || npc.getId() == 1598
					...
					OldMagicSystem.sendObjectTeleportSpell(player, true, new WorldTile(2910, 4832, 0));
