...
import com.rs.game.minigames.hunter.PuroPuro;
...
import com.rs.game.player.content.GraveStoneHandler;
...
public class ButtonHandler {
	...
	public static void handleButtons(final Player player, InputStream stream, final int packetId) {
		...
		if(interfaceId == 1019) {
			ReferSystem.handleButtons(interfaceId, componentId, player);
		}
			...
			if (componentId == 77 || componentId == 101) {
				...
				if (tab == null || tab == 0) {
					...
					if(player.isMember() || player.isSuperMember() || player.isExtremeMember() || player.isLegendaryMember()) {
						TeleportTab.open(player);
					}else {
						player.sm("This functionality is only available to members");
					}
				} else if (tab == 2) {
					SettingsTab.open(player);
				} else if (tab == 3) {
					if(player.isExtremeMember() || player.isLegendaryMember())
						GearTab.open(player, null);
					player.sm("This functionality is only available to extreme members.");
				} else if (tab == 4) {
			...
			if (componentId == 22) {
				...
				player.sm(player.isFilteredProfanity() ? "CAUTION! BETA." : null);
...

		{
			...
			if (packetId == 14) {
				...
				case 54: // Dung
					...
					if (player.getTemporaryAttributtes().remove("leveledUp[24]") != Boolean.TRUE) {
						...
						lvlupSkill = 24;
						player.getPackets().sendConfig(1230, 707);// TODO find correct id for dungeoneering levelup
					}
					break;
				}
				// player.getPackets().sendIComponentText(741, 3, "");
			...
			if (packetId == 14) {
				...
				case 120: // Dung
					...
					if (player.getTemporaryAttributtes().remove("leveledUp[24]") != Boolean.TRUE) {
						...
						for (int i = 705; i < 800; i++)
							player.getPackets().sendConfig(1230, i);
					}
					break;
				}
				// player.getPackets().sendIComponentText(741, 3, "");
				...
				if(player.getRights() == 2) {
					...
					if (player.getRights() < 2 && (skillId > 6 && skillId <= 24)) {
						player.getPackets().sendGameMessage(
								"You can only set attack, strength, defence, range, magic, prayer and hitpoints.");
						return;
					}
					...
					player.getPackets().sendInputIntegerScript(true,
							"Please enter " + (usingLevel ? "level" : "xp") + " you want to set: ");
				}
			} else if (packetId == 55) {
				if(player.getRights() == 2) {
					// clear level
					...
					if (player.getRights() < 2 && (skillId > 6 && skillId <= 24)) {
						player.getPackets().sendGameMessage(
								"You can only clear attack, strength, defence, range, magic, prayer and hitpoints.");
						return;
					}
					Limits i = Limits.forId(skillId);
					if (i != null) {
						if (player.getSkills().getLevelForXp(skillId) > WorldPacketsDecoder.getMaxedLevel(player,
								skillId)) {
							WorldPacketsDecoder.setMaxedLevel(player, skillId, player.getSkills().getLevelForXp(skillId));
						}
						if (WorldPacketsDecoder.getMaxedLevel(player, skillId) > i.getLevel()) {
							int level = WorldPacketsDecoder.getMaxedLevel(player, skillId);
							player.getSkills().setXp(i.getSkill(), Skills.getXPForLevel(level));
							player.getSkills().set(i.getSkill(), level);
						} else {
							player.getSkills().setXp(i.getSkill(), Skills.getXPForLevel(i.getLevel()));
							player.getSkills().set(i.getSkill(), i.getLevel());
						}
					}
				}
			}
			...
			if (componentId == 6) {
				...
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON5_PACKET) {
					...
					if (capeId == 20769 || capeId == 20771) {// add wilderness
																// block after
																// level 20
						OldMagicSystem.sendJewerlyTeleportSpell(player, true, 9603, 1684, 4, new WorldTile(3054, 3511, 0));// edge
																													// monestary
																													// teleport
					}
				...
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON2_PACKET) {
					...
					if (amuletId <= 1712 && amuletId >= 1706) {
						if (OldMagicSystem.sendJewerlyTeleportSpell(player, true, Transportation.EMOTE, Transportation.GFX, 4,
				...
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON2_PACKET) {
					...
					if (player.getEquipment().getWeaponId() >= 18349 && player.getEquipment().getWeaponId() <= 18358)
						player.getCharges().checkPercentage("Charges left: ", player.getEquipment().getWeaponId(),
								false);
				} else if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET)
				...
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET) {
					...
					if (player.getEquipment().getShieldId() >= 18359 && player.getEquipment().getShieldId() <= 18364)
						player.getCharges().checkPercentage("Charges left: ", player.getEquipment().getShieldId(),
								false);
				} else if (packetId == WorldPacketsDecoder.ACTION_BUTTON8_PACKET) {
				...
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON3_PACKET) {
					...
					if (ringId == 15707){//ring of dungeon
						if (!player.getControlerManager().processItemTeleport(new WorldTile(3449, 3698, 0)))
							return;
							WorldTasksManager.schedule(new WorldTask() {
							int loop;
				...
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON2_PACKET) {
					...
					if (player.getEquipment().getRingId() == 2550)
						player.getPackets()
								.sendGameMessage("Your ring of recoil has " + player.getRecoilHits() + " left.");
					if (player.getEquipment().getWeaponId() == 18349)
						player.getCharges().checkCharges("Current charges", 18349);
				} else if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET) {
...
/*
	...
	*/
	public static void sendItemsKeptOnDeath(Player player, boolean wilderness) {
		...
		if (slots[0].length > 0) {
		    for (int i = 0; i < slots[0].length; i++)
			player.getVarsManager().setVarBit(9222 + i, slots[0][i]);
		    player.getVarsManager().setVarBit(9227, slots[0].length);
		} else {
		    player.getVarsManager().setVarBit(9222, -1);
		    player.getVarsManager().setVarBit(9227, 1);
		}
		player.getVarsManager().setVarBit(9226, wilderness ? 1 : 0);
		player.getVarsManager().setVarBit(9229, skulled ? 1 : 0);
		StringBuffer text = new StringBuffer();
		...
		if (wilderness) {
		    ...
		    int time = GraveStoneHandler.getMaxAmountOnTimer(player.getGraveStone());
	...
	public static void refreshRunePouch(Player player) {
		Item[] items = player.getRunePouch().getItems();
		player.getInterfaceManager().sendInterface(1284);
		player.getInterfaceManager().sendInventoryInterface(670);
	...
	public static void storeRunePouch(Player player, Item item, int amount) {
		...
		if (!OldMagicSystem.isRune(newItem.getId())) {
