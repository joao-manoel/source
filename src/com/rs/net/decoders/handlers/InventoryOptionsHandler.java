...
import com.rs.game.minigames.hunter.FlyingEntityHunter;
import com.rs.game.minigames.hunter.FlyingEntityHunter.FlyingEntities;
...
public class InventoryOptionsHandler {
	...
	public static void handleItemOption1(Player player, final int slotId, final int itemId, Item item) {
				...
				if (itemId == 3691) {
					WorldTasksManager.schedule(new WorldTask() {
						...
						public void run() {
							ticks++;
							if (ticks == 1) {
								  player.gfx(new Graphics(1682));
								player.animate(new Animation(1320));
							} else if (ticks == 2) {
								FadingScreen.fade(player, new Runnable() {
								    @Override
								    public void run() {
								    	 player.setNextWorldTile(new WorldTile(2626, 3599, 0));
								    }
								});
							} else if (ticks == 3) {
								 player.getInventory().deleteItem(3691, 1);
					    	 player.getInventory().addItem(3690, 1);
					}}	
			}, 0, 1);
		    }
		if(itemId == 24855 || itemId == 24857 || itemId == 24859 || itemId == 24861) {
			player.getDialogueManager().startDialogue("MemberShip", itemId);
		}
		...
		if ((itemId >= 1741 && itemId <= 1743)) {
			if (!player.getInventory().containsItem(1733, 1) && !player.getToolbelt().contains(1733))
				...
				if(craft != null)
					player.getDialogueManager().startDialogue("LeatherCraftingD", itemId);
			}
		}*/
		
		//Armadyl staff create orb
		if(itemId == 21776) {
			if(player.getInventory().containsItem(21776, 100)) {
				player.animate(new Animation(2721));
				player.getInventory().deleteItem(21776, 100);
				player.getInventory().addItem(21775, 1);
				return;
			}else {
				player.sm("You need 100 shards to create an orb");
			}
		...
		if (itemId == 15084) {
			...
			player.sm("You do not have permission.");
			return;
		}
		...
		if (itemId >= 5509 && itemId <= 5514) {
		 ...
		 if (LeatherCrafting
	                .handleItemOnItem(player, LeatherCrafting.NEEDLE, item)) {
	            return;
	        }
	...
	public static void handleItemOnItem(final Player player, InputStream stream) {
		...
		if (interfaceId == Inventory.INVENTORY_INTERFACE && interfaceId == interfaceId2
			...
			if (itemUsed.getId() == LeatherCrafting.NEEDLE.getId()
				...
				if(itemUsed.getId() == 24374 || usedWith.getId() == 24374) {
					player.sm("teste");
				}
                if (LeatherCrafting
			...
			if(itemUsed.getId() == 21775 && usedWith.getId() == 1391
					|| itemUsed.getId() == 1391 && usedWith.getId() == 21775) {
				player.animate(new Animation(1367));
			...
			//ganodermic poncho
			if (
					itemUsed.getId() == 22451 && usedWith.getId() == 22456
					||
					itemUsed.getId() == 22456 && usedWith.getId() == 22451
				) {
			...
			player.getPackets().sendGameMessage("You create a Ganodermic Poncho.");
			return;
			} else {
			player.getPackets().sendGameMessage("You do not have enough flakes.");
		...
		//ganodermic visor
			if (
					itemUsed.getId() == 22451 && usedWith.getId() == 22452
					||
					itemUsed.getId() == 22452 && usedWith.getId() == 22451
				) {
			...
			player.getPackets().sendGameMessage("You create a Ganodermic Visor.");
			return;
			} else {
			player.getPackets().sendGameMessage("You do not have enough Flakes.");
		...
		//gano leggings    
			if (
					itemUsed.getId() == 22451 && usedWith.getId() == 22454
					||
					itemUsed.getId() == 22454 && usedWith.getId() == 22451
					) {
				...
				player.getPackets().sendGameMessage("You create Ganodermic Leggings.");
				return;
			} else {
			player.getPackets().sendGameMessage("You do not have enough flakes.");			
			return;
		...
		//polypore staff
			if (
					...
					||
					itemUsed.getId() == 22498 && usedWith.getId() == 22448
					) {
			...
			player.getPackets().sendGameMessage("You create a Polypore Staff.");
			return;
			} else {
			player.getPackets().sendGameMessage("You do not have all the required items.");
			...
			//explorer ring 1
			if (usedWith.getId() == 13560) {
				if (player.explorerRing1 > 1)  {	 
					player.getPackets().sendGameMessage("It seems you already used your charge, try again later.");
					return;
				}
					if (player.getRunEnergy() < 50) {
					player.animate(new Animation(9988));
				 player.gfx(new Graphics(1733));
				player.setRunEnergy(player.getRunEnergy() + 50);
				player.explorerRing1 = 32400000; //9h
				}
	...
	public static void handleItemOption3(Player player, int slotId, int itemId, Item item) {
		...
		if (itemId == 20767 || itemId == 20769 || itemId == 20771)
			...
			player.getCharges().checkPercentage("Charges:", itemId, false); //charges
		else if(itemId == 20137) {
			...
			return;
		}else if (itemId == 15707){//ring of dungeon
