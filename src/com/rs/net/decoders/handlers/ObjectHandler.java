...
import com.rs.game.player.actions.ObjectActionTeleport;
	...
	private static void handleOption1(final Player player, final WorldObject object, InputStream stream) {
		...
		if(Settings.DEBUG) {
			if(player.getRights() == 2) {
				player.sm("DEBUG: OBJECT ID "+object.getId());
			}
		}
		...
		if (ObjectOption1.validObject(player, object))
			...
			int xpbase = player.isLegendaryMember() ? 4000 : player.isExtremeMember() ? 3500 : player.isSuperMember() ? 3000 : player.isMember() ? 2500 : 2000;
			int ia = dl * xpbase;
			
			
			player.getSkills().addXp(24, ia);
            player.stopAll();
            
            player.getAppearence().generateAppearenceData();
		    player.setDungeonToken(player.getDungeonToken() + (ia/10));
            player.getPackets().sendGameMessage("<col=960000>You've completed the dungeon and earned "+ ia +" experience and "+ (ia/10) +"  Tokens.");
		...
		if(id == 1317 || id == 68973 || id == 26723) {
			if (objectDef.containsOption(1, "Teleport"))
				SpiritTreeTransport.sendInterface(player);	
		}
		...
		if(id == 5083) {
			//brimhaven dungeon enter cave
			player.setNextWorldTile(new WorldTile(2713, 9564, 0));
			return;
		}
		
		if(id == 5084) {
			//brimhaven dungeon exit cave
			player.setNextWorldTile(new WorldTile(2745, 3152, 0));
			return;
			//
		}
		
		if(id == 8966) {
			//sair da caverna - waterbirth
			player.setNextWorldTile(new WorldTile(2524, 3739, 0));
			return;
			//
		}
		
		if(id == 8929) {
			//entrada da caverna - waterbirth
			player.setNextWorldTile(new WorldTile(2442, 10147, 0));
			return;
		}
		
		if(id == 4500) {
			//sair da caverna - fremennik
			player.setNextWorldTile(new WorldTile(2795, 3616, 0));
			return;
		}
		
		if(id == 34395) {
			//entrar da caverna - fremennik
		...
		if(id == 12327) {
			//sair da caverna - jadinko lair
			player.setNextWorldTile(new WorldTile(2946, 2954, 0));
			return;
		}
		
		if(id == 12328) {
			//entrar da caverna - jadinko lair
			player.setNextWorldTile(new WorldTile(3011, 9275, 0));
			return;
		}
		...
		if (id == 213 || id == 214) {
			AgilityFA.FaladorGrappleWall(player, object);
			return;
				}
		
		if (id == 9309 || id == 9310) {
			AgilityFA.FaladorTunnel(player, object);
			return;
				}
		
		if (id == 17051 || id == 17052) {
			AgilityFA.FaladorJumpDown(player, object);
			return;
		}
		
		if (id == 11844) {
			AgilityFA.FaladorCrumbledWall(player, object);
			return;
		}
		...
		player.setRouteEvent(new RouteEvent(object, new Runnable() {
			...
			public void run() {
				...
				if (id == 50552) 
			 		player.setNextWorldTile(new WorldTile(3454,3725,0));
				if (id == 11724) {
				...
				else if (id == 57225) { //nex entrance
                    player.getDialogueManager().startDialogue("NexEntrance");
				} 
				else if (id == 2406 || id == 14097) {
				...
				if (object.getId() == 47237 && player.getY() == 5260) {
					...
					if(player.getPlane() == 2) {
						player.getActionManager().setAction(new ObjectActionTeleport(new WorldTile(3106, 3160, 1), 2));
					}else {
				...
				// start of varrock dungeon
					...
					player.sm("disabled 1");
					return;
					//player.getCutscenesManager().play("DTPreview");
				}
				
				if(id == 62674) {
					OldMagicSystem.pushLeverTeleport(player, new WorldTile(3743, 6406, 0));
				}
					
				
				else if (id == 62681) {
					player.sm("disabled 2");
					return;
...
//					player.getDominionTower().openModes();
					...
					player.sm("disabled 4");
					return;
				//	player.getDialogueManager().startDialogue("DTClaimRewards");
				}
				
				 else if (id == 62677) {
					if(player.getHasGloves()) {
						player.getDialogueManager().startDialogue("DominionReward"); // face
							return;
						}
						 
						player.getDialogueManager().startDialogue("buyDominionReward");
						return;
				 }
					...
					case "soft clay":
						player.getActionManager().setAction(
								new Mining(object, RockDefinitions.SOFT_CLAY));
						break;
