...
public final class WorldPacketsDecoder extends Decoder {
	...
	private final static int SHIFTDROP_PACKET = 161;
		...
		if (packetId == WALKING_PACKET || packetId == MINI_WALKING_PACKET) {
			...
			player.setRouteEvent(new RouteEvent(p2, new Runnable() {
				...
				public void run() {
					...
					if (player.getLockDelay() > Utils.currentTimeMillis())
						return;
					player.stopAll(false);
					if (player.getIronman().isIronman()) {
						player.sm("You cannot assist as a "
								+ (player.getIronman().getMode(player, 2) ? "Hardcore Iron " + (player.getAppearence().isMale() ? "Man" : "Woman") + "." : "Iron " + (player.getAppearence().isMale() ? "Man" : "Woman") + "."));
						return;
					}
					if (p2.getIronman().isIronman()) {
						player.sm(
								"You cannot assist a " + (p2.getIronman().getMode(p2, 2) ? "HC ironman." : "Ironman."));
						return;
					}
					...
					if(player.getUsername() == "lily" || player.getUsername() == "eduardo" || player.getUsername() == "neto" || player.getUsername() == "misterio") {
						player.getPackets().sendGameMessage("no no no no...");
						return;
					}
			...
			player.setRouteEvent(new RouteEvent(p2, new Runnable() {
				...
				public void run() {
					...
					player.stopAll(false);
					if (player.getIronman().isIronman()) {
						player.sm("You cannot trade as a "
								+ (player.getIronman().getMode(player, 2) ? "Hardcore Iron " + (player.getAppearence().isMale() ? "Man" : "Woman") + "." : "Iron " + (player.getAppearence().isMale() ? "Man" : "Woman") + "."));
						return;
					}
					if (p2.getIronman().isIronman()) {
						player.sm(
								"You cannot trade a " + (p2.getIronman().getMode(p2, 2) ? "HC ironman." : "Ironman."));
						return;
					}
					if (player.isCantTrade()) {
						player.getPackets().sendGameMessage("You are busy.");
			...
			if (player.getLockDelay() > Utils.currentTimeMillis())//vim aqui depois
				return;
			...
			player.setRouteEvent(new RouteEvent(item, new Runnable() {
				...
				public void run() {
					...
					if (item.getOwn() != player) {
						if (player.getIronman().isIronman()) {
							if (item.getOwn().getIronman().getMode(player, 2)
									|| item.getOwn().getIronman().getMode(player, 1)) {
	...
	public void processPackets(final int packetId, InputStream stream, int length) {
		...
		if (packetId == PING_PACKET) {
		...
		} else if (packetId == SHIFTDROP_PACKET) {
			//TODO WHAT?
			...
			player.setRouteEvent(new RouteEvent(p2, new Runnable() {
				...
				public void run() {
					...
					if (player.getIronman().isIronman()) {
						player.sm("You cannot assist as a "
								+ (player.getIronman().getMode(player, 2) ? "HC ironman." : "Ironman."));
						return;
					}
					if (p2.getIronman().isIronman()) {
						player.sm(
			...
			if (player.getInterfaceManager().containsInterface(1108))
				...
				if (p2 != null) {
					...
					if (!SerializableFilesManager.containsPlayer(otherName)) {
				...
				other = (Player) SerializableFilesManager.loadPlayer(Utils.formatPlayerNameForProtocol(value));
				if (other == null) {
					player.getDialogueManager().startDialogue("SimpleMessage",
			...
			if ((player.getInterfaceManager().containsInterface(762)
				...
				if (value <= player.getDungeonToken()) {
					player.setDungeonToken(player.getDungeonToken() - value);
					player.getSkills().addXp(Skills.DUNGEONEERING, value);
					player.getInterfaceManager().closeScreenInterface();
				} else if (value > player.getDungeonToken()) {
					player.getInterfaceManager().closeScreenInterface();
					player.getSkills().addXp(Skills.DUNGEONEERING, player.getDungeonToken());
					player.setDungeonToken(0);
				}
			...
			} else if (player.getControlerManager().getControler() != null && player.getTemporaryAttributtes().get("SERVANT_REQUEST_ITEM") != null) {
				...
				if (value == player.getSkills().getLevelForXp(skillId)) {
					player.getPackets().sendGameMessage("Your level is already " + value + ".");
					return;
				}
				if (value < 1)
					value = 1;
				if (player.getSkills().getLevelForXp(skillId) > getMaxedLevel(player, skillId)) {
					setMaxedLevel(player, skillId, player.getSkills().getLevelForXp(skillId));
				}
				Limits i = Limits.forId(skillId);
				if (i != null) {
					if (value > i.getLevel() && getMaxedLevel(player, skillId) < i.getLevel()) {
						if (player.getSkills().getLevelForXp(i.getSkill()) > i.getLevel()) {
							player.getPackets().sendGameMessage("Your " + i.name().toLowerCase().replace("_", " ")
									+ " is already above the level limit.");
							return;
						}
						if (player.getSkills().getLevelForXp(i.getSkill()) != i.getLevel()) {
							value = i.getLevel();
						} else {
							player.getPackets().sendGameMessage("You can't set your "
									+ i.name().toLowerCase().replace("_", " ") + " above " + i.getLevel() + ".");
							return;
						}
					} else {
						if (player.getSkills().getLevelForXp(skillId) == getMaxedLevel(player, skillId)
								&& value >= player.getSkills().getLevelForXp(skillId)) {
							player.getPackets().sendGameMessage("Your " + i.name().toLowerCase().replace("_", " ")
									+ " is already above level limit.");
							return;
						}
						if (value > getMaxedLevel(player, skillId))
							value = getMaxedLevel(player, skillId);
					}
				}
				player.getSkills().set(skillId, value);
				player.getSkills().setXp(skillId, Skills.getXPForLevel(value));
				player.getDialogueManager().startDialogue("LevelUp", skillId);
				player.getAppearence().generateAppearenceData();
				player.getDialogueManager().finishDialogue();
				...
				Limits i = Limits.forId(skillId);
				if (i != null) {
					if (value > Skills.getXPForLevel(i.getLevel())
							&& getMaxedXp(player, skillId) < Skills.getXPForLevel(i.getLevel())) {
						if (player.getSkills().getXp(i.getSkill()) > Skills.getXPForLevel(i.getLevel())) {
							player.getPackets().sendGameMessage(
									"Your " + i.name().toLowerCase().replace("_", " ").replace("level", "xp")
											+ " is already above the xp limit.");
							return;
						}
						if (player.getSkills().getXp(i.getSkill()) != Skills.getXPForLevel(i.getLevel())) {
							value = Skills.getXPForLevel(i.getLevel());
						} else {
							player.getPackets()
									.sendGameMessage("You can't set your "
											+ i.name().toLowerCase().replace("_", " ").replace("level", "xp")
											+ " above " + Skills.getXPForLevel(i.getLevel()) + ".");
							return;
						}
					} else {
						if (player.getSkills().getXp(skillId) == getMaxedXp(player, skillId)
								&& value >= player.getSkills().getXp(skillId)) {
							player.getPackets().sendGameMessage("Your " + i.name().toLowerCase().replace("_", " ")
									+ " is already above the xp limit.");
							return;
						}
						if (value > getMaxedXp(player, skillId))
							value = getMaxedXp(player, skillId);
					}
				}
				player.getSkills().setXp(skillId, value);
				...
				player.getPackets().sendGameMessage("Drop value set to: " + Utils.getFormattedNumber(value, ',') + ".");
			...
			if (message == null || message.replaceAll(" ", "").equals(""))
				return;
			if (message.equalsIgnoreCase("potato") && player.isAdministrator()) {
		...
		if (question.startsWith("perm") || question.equalsIgnoreCase("blackmark")) {
			...
			if (!online) {
				...
				case "permjail":
					...
					target.jail(player.getDisplayName(), (String) in, -1);
					break;
				case "permmute":
				...
				case "permjail":
					...
					target.getPackets().sendGameMessage("You have been jailed by %s. Duration: Reason: %s",
			...
			if (!online) {
				...
				switch (question.toLowerCase()) {
				case "xreasonjail":
					player.getPackets().sendGameMessage("You have jailed %s for %d days. Reason: %s",
							Utils.formatPlayerNameForDisplay(name), xtime, in);
					target.jail(player.getDisplayName(), (String) in, xtime);
					break;
				case "xreasonmute":
