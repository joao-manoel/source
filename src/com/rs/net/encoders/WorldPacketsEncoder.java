...
public class WorldPacketsEncoder extends Encoder {
	...
	public void sendShiftDropPacket() {
		OutputStream stream = new OutputStream(2);
		stream.writePacket(player, 161);
		stream.writeByte(player.isShiftDrop() ? 1 : 0);
		session.write(stream);
	}

	public void sendSlowDragPacket() {
		OutputStream stream = new OutputStream(2);
		stream.writePacket(player, 162);
		stream.writeByte(player.isSlowDrag() ? 1 : 0);
		session.write(stream);
	}

	public void sendZoomPacket() {
		OutputStream stream = new OutputStream(2);
		stream.writePacket(player, 163);
		stream.writeByte(player.isZoom() ? 1 : 0);
		session.write(stream);
	}
	...
	public void sendHintIcon(HintIcon icon) {
		...
		if (icon.getTargetType() == 0)
			...
			if (icon.getTargetType() == 1 || icon.getTargetType() == 10) {
				...
				stream.writeByte(icon.getPlane());
	...
	public void sendPanelBoxMessage(String text) {
		sendMessage(player.isAdministrator() ? 99 : 0, text, null);
	}
