...
public final class LoginPacketsEncoder extends Encoder {
	...
	public void sendLobbyDetails(Player player) {
		...
		stream.writeByte(0);
		stream.writeByte(0);
		stream.writeLong(10); //members subscription end
		stream.write5ByteInteger(12);
		stream.writeByte(player.isMember() ? 0x2 : 0x0); //0x1 - if members, 0x2 - subscription
		stream.writeInt(1); 
		stream.writeByte(1); 
		...
		stream.writeByte(player.getEmail() == null ? 0 : 3); //email status (0 - no email, 1 - pending parental confirmation, 2 - pending confirmation, 3 - registered)
		stream.writeShort(0);
		stream.writeShort(0);
		stream.writeByte(0);
