...
public final class ItemDefinitions {
	...
	public HashMap<Integer, Integer> getWearingSkillRequiriments() {
		...
		if (itemRequiriments == null) {
			...
			if(getId() == 20068) {
				...
				itemRequiriments.put(Skills.CONSTRUCTION, 64);
				itemRequiriments.put(Skills.CRAFTING, 68);
				itemRequiriments.put(Skills.MAGIC, 70);
			}
			if(getId() == 22424) {
				itemRequiriments.put(Skills.PRAYER, 60);
				itemRequiriments.put(Skills.CRAFTING, 60);
			}
			if(getId() == 20767 || getId() == 20768) {
				...
				itemRequiriments.put(Skills.CONSTRUCTION, 99);
	...
	public int getDropPrice() {
		try {
			for (String lines : FileUtilities.readFile("./data/items/drop_price.txt")) {
				String[] data = lines.split(" - ");
