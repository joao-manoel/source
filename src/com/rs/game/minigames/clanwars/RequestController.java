...
public final class RequestController extends Controler {
	...
	public boolean processButtonClick(int interfaceId, int componentId, int slotId, int packetId) {
		ClanWars clanWars = (ClanWars) player.temporaryAttribute().get("clan_wars");
		if (clanWars == null) {
			return true;
		}
		...
		case 791:
			...
			case 40:
				...
				return false;
			case 43:
				clanWars.setVictoryType(0x3e8, player, other);
				return false;
			case 46:
				clanWars.setVictoryType(0x9c4, player, other);
				return false;
			case 49:
				clanWars.setVictoryType(0x1388, player, other);
				return false;
			case 52:
				clanWars.setVictoryType(0x2710, player, other);
				return false;
			case 56: // Most kills.
			...
			case 60:
				...
				return false;
			case 63:
				clanWars.setTimeLeft(0x3e8, player, other);
				return false;
			case 66:
				clanWars.setTimeLeft(0xbb8, player, other);
				return false;
			case 69:
				clanWars.setTimeLeft(0x1770, player, other);
				return false;
			case 72:
				clanWars.setTimeLeft(0x2328, player, other);
				return false;
			case 75:
				clanWars.setTimeLeft(0x2ee0, player, other);
				return false;
			case 78:
				clanWars.setTimeLeft(0x3a98, player, other);
				return false;
			case 81:
				clanWars.setTimeLeft(0x4650, player, other);
				return false;
			case 84:
				clanWars.setTimeLeft(0x5dc0, player, other);
				return false;
			case 87:
				clanWars.setTimeLeft(0x7530, player, other);
				return false;
			case 90:
				clanWars.setTimeLeft(0x8ca0, player, other);
				return false;
			case 93:
				clanWars.setTimeLeft(0xbb80, player, other);
				return false;
			case 100:
	...
	public boolean canPlayerOption1(final Player target) {
		...
		player.setCoordsEvent(new CoordsEvent(target, new Runnable() {
			...
			public void run() {
				...
				player.temporaryAttribute().put("clan_request_p", target);
				if (target.temporaryAttribute().get("clan_request_p") == player) {
	...
	public boolean processObjectClick1(WorldObject object) {
		...
		case 28213:
			 ...
			 * return false; }
			 */
			player.getPackets().sendGameMessage("This feature is currently disabled.");
	...
	public void decline() {
		Player other = (Player) player.temporaryAttribute().get("clan_request_p");
		ClanWars war = (ClanWars) player.temporaryAttribute().get("clan_wars");
		...
		if (war != null) {
			...
			player.temporaryAttribute().remove("clan_request_p");
			other.temporaryAttribute().remove("clan_request_p");
			player.temporaryAttribute().remove("clan_wars");
			other.temporaryAttribute().remove("clan_wars");
		}
	}
			...
			public void run() {
				if (loop == 0) {
					...
					if (killer != null) {
						...
						System.out.println("Request: " + killer.getUsername() + " " + player.getUsername() + "");
						System.out.println("Request: " + killer == null);
