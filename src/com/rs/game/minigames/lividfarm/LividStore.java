...
public abstract class LividStore {
	...
	public enum Wish {
		...
		LET_IT_RAIN_SEEDS(18000) {
			...
			public void process(Player player) {
				...
				for (int ids : seeds) {
					World.updateGroundItem(new Item(ids, Utils.random(7, 11)), Utils.getFreeTile(player, 3), player,
							60, 0);
				}
		...
		LET_IT_RAIN_AWESOME_SEED(55000) {
			...
			public void process(Player player) {
				...
				for (Item item : seeds) {
					World.updateGroundItem(item, Utils.getFreeTile(player, 3), player, 60, 0);
				}
				removeProduce(player);
				player.getDialogueManager().startDialogue("SimpleMessage",
