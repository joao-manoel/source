...
public class ThievesGuildChests extends Action {
	...
	public boolean start(Player player) {
		if (checkAll(player)) {
			...
			player.lock();
			player.animate(PICKLOCK_CHEST_ANIMATION);
	...
	private boolean checkAll(Player player) {
		...
		if (object.getId() == 52299 && !player.getInventory().containsItem(1523, 1)) {
    		player.sm("You attempt to pick the lock.");
	...
	public static boolean handleChest(Player player, WorldObject object, long timer) {
		...
		World.spawnObjectTemporary(openedChest, timer, true, false);
		return true;			
	}
