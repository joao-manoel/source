...
public final class Mining extends MiningBase {
	...
	public static enum RockDefinitions {
		...
		SOFT_CLAY(75, 5, 1761, 10, 1, 94217, 5, 0),
		Iron_Ore(15, 35, 440, 15, 1, 11552, 10, 0), Sandstone_Ore(
	...
	private int getMiningDelay(Player player) {
		...
		if (goldenpickaxe(player)) {
			mineTimer = 1;
		}
		return mineTimer;
	}

	private boolean goldenpickaxe(Player player) {
		int weaponId = player.getEquipment().getWeaponId();
		if (weaponId != -1) {
			switch (weaponId) {
			case 20786: // gilded dragon pick
					return true;
			}
	...
	private void addOre(Player player) {
		...
		if (definitions.getOreId() != -1) {
			...
			if(definitions.getOreId() == 444)
				player.getTaskManager().checkComplete(Tasks.MINE_GOLD_ORE);
			...
			if(definitions.getOreId() == 453)
				player.getTaskManager().checkComplete(Tasks.MINE_COAL);
		}
	}
	
	private boolean doubleLoot(Player player) {
		int chance = 6 + player.getPlayerMode() * 2;
		if (Utils.getRandom(100) <= chance && player.getPlayerMode() > 0) {
