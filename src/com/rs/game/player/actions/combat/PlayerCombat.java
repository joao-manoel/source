...
public class PlayerCombat extends Action {
	...
	private void addAttackedByDelay(Entity player) {
		...
		target.setAttackedByDelay(Utils.currentTimeMillis() + 5000); // 5seconds
	}
	...
	private int getRangeCombatDelay(Player player, int weaponId, int attackStyle) {
		...
		if (weaponId != -1) {
			...
			if (weaponName.contains("training bow") || weaponName.contains("shortbow") || weaponId == 4734
		...
		if (player.getCombatDefinitions().getSpellBook() == 192) {
			...
			if (spell == ModernCombatSpellsStore.CRUMBLE_UNDEAD) {
				...
				if (target instanceof NPC) {
					...
					if (!undead || (target instanceof Player)) {
						player.getPackets().sendGameMessage("You can only cast crumble undead on undead monsters.");
						return -1;
					}
				}
			}
			if (spell == ModernCombatSpellsStore.BIND || spell == ModernCombatSpellsStore.SNARE
				...
				if (target.getFreezeDelay() > Utils.currentTimeMillis()) {
					player.getPackets().sendGameMessage("This " + (target instanceof Player ? "player" : "monster")
							+ " is already effected by this spell.", true);
					return -1;
				}
			}
				...
				if (p2.getEquipment().getWeaponId() == 24203 && p2.getRunicStaff().getSpellId() == spell.getId()) {
					if (Utils.getRandom(4) == 0) {
						...
						player.getPackets().sendGameMessage(p2.getDisplayName() + " absorbs your "
								+ spell.name().toLowerCase().replace("_", " ") + " spell.");
						p2.getPackets().sendGameMessage("You absorb the " + spell.name().toLowerCase().replace("_", " ")
			...
			return spell.getId() == 99 && player.getEquipment().getWeaponId() == 21777 ? 3 : 4;
	...
	public static boolean hasTokkulZoEffect(Player player, Entity target) {
		if (target instanceof NPC) {
			NPC n = (NPC) target;
			if (player.getEquipment().getRingId() == 23643 && target instanceof NPC
					&& (n.getDefinitions().getName().startsWith("TzTok-")
							|| n.getDefinitions().getName().startsWith("Ket-")
							|| n.getDefinitions().getName().startsWith("Yt-")
							|| n.getDefinitions().getName().startsWith("Tok-")
							|| n.getDefinitions().getName().startsWith("Tz-")
							|| n.getDefinitions().getName().startsWith("Jal")
							|| n.getDefinitions().getName().startsWith("TokHaar-")
							|| n.getDefinitions().getName().startsWith("Har-")))
				return true;
		}
		return false;
	...
	public int getMagicMaxHit(Player player, int baseDamage) {
		...
		if (target instanceof NPC) {
			NPC n = (NPC) target;
			if (player.getSlayerManager().getCurrentTask() != null || n.getId() == 4474) {
				if (player.getSlayerManager().isValidTask(n.getName()) || n.getId() == 4474) {
					if (FullSlayerHelmet(player))
						attack *= 1.20;
					if (Hexcrest(player))
						attack *= 1.15;
				}
			}
			if (player.getAuraManager().isActivated() && !Hexcrest(player) && !FullSlayerHelmet(player))
				attack *= player.getAuraManager().getMagicAccurayMultiplier();
		}
		...
		if (target instanceof Player) {
			...
			defence = (n.getBonuses()[CombatDefinitions.NPC_MAGIC_LEVEL] * 0.7)
					+ (n.getBonuses()[CombatDefinitions.NPC_MAGIC_DEFENCE] * 0.3);
			defence = Math.round(defence);
			defence += 8;
		...
		if (target instanceof NPC) {
			...
			if (player.getSlayerManager().getCurrentTask() != null || n.getId() == 4474) {
				if (player.getSlayerManager().isValidTask(n.getName()) || n.getId() == 4474) {
					...
					if (Hexcrest(player))
						max_hit *= 1.15;
					if (player.getEquipment().getRingId() >= 15398 && player.getEquipment().getRingId() <= 15402)
						max_hit += 40;
				}
			}
	...
	public void MeleeHitDelay(final Player player) {
		...
		if (weaponId == 1419) {
			...
			int hit = getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.0, true);
			if (hit > 100) {
				hits = new int[] { Utils.getRandom(hit), Utils.getRandom(hit / 2), Utils.getRandom((hit / 2) / 2) };
			} else {
				hit = getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.0, true);
				if (hit > 100) {
					hits = new int[] { 0, Utils.getRandom(hit / 2), Utils.getRandom((hit / 2) / 2) };
				} else {
					hit = getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.0, true);
					if (hit > 100) {
						hits = new int[] { 0, 0, Utils.getRandom((hit / 2) + 10) };
					} else {
						hit = getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.0, true);
						if (hit > 100) {
							hits = new int[] { 0, 0, Utils.getRandom((int) (hit * 1.0)) };
						} else {
							int[] miss = { 0 };
		...
		if (defenceAffects) {
			...
			if (!ranging) {
				...
				if (target instanceof NPC) {
					NPC n = (NPC) target;
					if (n.getId() == 4474) {
						if (SlayerHelmet(player) || FullSlayerHelmet(player))
							attack *= 1.20;
						if (BlackMask(player))
							attack *= 1.15;
					}
					...
					if (player.getAuraManager().isActivated() && !BlackMask(player) && !FullSlayerHelmet(player)
							&& !SlayerHelmet(player))
						attack *= player.getAuraManager().getAttackAccurayMultiplier();
				}
				...
				if (target instanceof NPC) {
					...
					if (player.getAuraManager().isActivated() && !FocusSight(player) && !FullSlayerHelmet(player))
						range *= player.getAuraManager().getRangeAccurayMultiplier();
				}
			...
			if (target instanceof Player) {
				...
				if (!ranging) {
					...
					defence = Math.round(defence);
					defence += 8;
					if (p2.getCombatDefinitions().getStyle(weaponId, attackStyle) == CombatDefinitions.DEFENSIVE)
						defence += 3;
					else if (p2.getCombatDefinitions().getStyle(weaponId, attackStyle) == CombatDefinitions.CONTROLLED)
						defence += 1;
					defence = defence * (1 + defenceBonus / 64);
					D = Math.round(defence);
				} else {
					...
					rangedefence += 8;
					if (p2.getCombatDefinitions().getStyle(weaponId, attackStyle) == CombatDefinitions.LONGRANGE
							|| p2.getCombatDefinitions().getStyle(weaponId, attackStyle) == CombatDefinitions.DEFENSIVE)
						defence += 3;
					else if (p2.getCombatDefinitions().getStyle(weaponId, attackStyle) == CombatDefinitions.CONTROLLED)
						defence += 1;
				...
				if (ranging) {
					...
					if (n.getTemporaryAttributtes().get("NPC_DEFENCE_LEVEL") != null)
						temporaryDef = (int) n.getTemporaryAttributtes().get("NPC_DEFENCE_LEVEL");
					rangedefence = temporaryDef > 0 ? temporaryDef
					...
					if (n.getTemporaryAttributtes().get("NPC_DEFENCE_LEVEL") != null)
						temporaryDef = (int) n.getTemporaryAttributtes().get("NPC_DEFENCE_LEVEL");
					defence = temporaryDef > 0 ? temporaryDef
							: n.getBonuses() != null ? n.getBonuses()[CombatDefinitions.NPC_DEFENCE_LEVEL]
									: n.getCombatLevel() / 3;
					defence = Math.round(defence);
					defence += 8;
	...
	private final int getMaxHit(Player player, int weaponId, int attackStyle, boolean ranging, boolean usingSpec,
		...
		if (!ranging) {
			...
			if (target instanceof NPC) {
				...
				if (undead) {
					if ((SalveAmmy(player) && !BlackMask(player) && !FullSlayerHelmet(player)
							&& !SlayerHelmet(player))) {
						maxHit *= 1.15;
					} else if (SalveAmmyE(player) && !BlackMask(player) && !FullSlayerHelmet(player)
							&& !SlayerHelmet(player)) {
						maxHit *= 1.20;
					}
				}
			...
			if (player.getCombatDefinitions().getStyle(weaponId, attackStyle) == CombatDefinitions.AGGRESSIVE)
				maxHit += 3;
			else if (player.getCombatDefinitions().getStyle(weaponId, attackStyle) == CombatDefinitions.CONTROLLED)
				maxHit += 1;
			return maxHit;
			...
			if (target instanceof NPC) {
				...
				if (player.getSlayerManager().getCurrentTask() != null || n.getId() == 4474) {
					if (player.getSlayerManager().isValidTask(n.getName()) || n.getId() == 4474) {
						if (FullSlayerHelmet(player))
							maxHit *= 1.20;
						else if (FocusSight(player))
							maxHit *= 1.15;
					}
				}
				if (player.getAuraManager().isActivated() && !FocusSight(player) && !FullSlayerHelmet(player))
					maxHit *= player.getAuraManager().getRangeAccurayMultiplier();
			}
			if (player.getCombatDefinitions().getStyle(weaponId, attackStyle) == CombatDefinitions.ACCURATE)
				maxHit += 3;
			return maxHit;
	...
	public void delayNormalHit(final int weaponId, final int attackStyle, final Hit... hits) {
		if (weaponId != 4153) {
			if (ItemDefinitions.getItemDefinitions(weaponId).getName().contains("maul")
	...
	private void delayHit(int delay, final int weaponId, final int attackStyle, final Hit... hits) {
		...
		@SuppressWarnings("unused")
		final String spell_Name = this.spell_name;
		...
		for (Hit hit : hits) {
			...
			if (target instanceof NPC) {
				...
				if (n.getId() == 1158) {
					if (hit.getLook() == HitLook.RANGE_DAMAGE || hit.getLook() == HitLook.MAGIC_DAMAGE) {
						hit.setDamage(0);
						player.getPackets().sendGameMessage(n.getName() + "s protection prayer blocks your "
								+ (hit.getLook() == HitLook.RANGE_DAMAGE ? "range" : "magic") + " attack!");
					}
				}
			...
			public void run() {
				for (Hit hit : hits) {
					...
					if (player == null || player.isDead() || player.hasFinished() || target.isDead()
						...
						return;
					if (target instanceof Player) {
						if (hit.getLook() == HitLook.MAGIC_DAMAGE || hit.getLook() == HitLook.RANGE_DAMAGE)
							doDefenceEmote(target);
						player.setDamage(hit.getDamage());
					...
					if (hit.getLook() == HitLook.MAGIC_DAMAGE && hit.getDamage() < 1) {
						splash = true;
						hit.setDamage(0);
					} else if (hit.getLook() == HitLook.MELEE_DAMAGE || hit.getLook() == HitLook.RANGE_DAMAGE
							|| hit.getLook() == HitLook.MAGIC_DAMAGE && hit.getDamage() > -1) {
						target.applyHit(hit);
					}
					// doDefenceEmote(player);
					int damage = hit.getDamage() > target.getHitpoints() ? target.getHitpoints() : hit.getDamage();
					if ((damage >= max_hit * 0.90) && (hit.getLook() == HitLook.RANGE_DAMAGE))
						hit.setCriticalMark();
					if ((damage >= max_hit * 0.90) && (hit.getLook() == HitLook.MAGIC_DAMAGE))
						hit.setCriticalMark();
					if ((damage >= max_hit * 0.90) && (hit.getLook() == HitLook.MELEE_DAMAGE))
						hit.setCriticalMark();
	...
	private boolean checkAll(Player player) {
		...
		size = 1;// if higher than 1 it hits from far away with melee lol
		int maxDistance = 16;
