...
public class OldMagicSystem {
	...
	public static final void processNormalSpell(Player player, int spellId, int packetId) {
		...
		case 33:
			...
			if (!OldMagicSystem.checkRunes1(player, true, OldMagicSystem.EARTH_RUNE, 2, OldMagicSystem.WATER_RUNE, 2, OldMagicSystem.NATURE_RUNE, 1)) {
		...
		case 65:
			...
			if (!OldMagicSystem.checkRunes1(player, true, OldMagicSystem.EARTH_RUNE, 4, OldMagicSystem.WATER_RUNE, 4, OldMagicSystem.NATURE_RUNE, 2)) {
				player.getPackets().sendGameMessage("You do not have enough runes for this spell.");
				return;
			}
			player.animate(new Animation(722));
	...
	public static void pushLeverTeleport(final Player player, final WorldTile tile) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			@Override
			public void run() {
				OldMagicSystem.sendObjectTeleportSpell(player, false, tile);
			}
		}, 1);
	}
	...
	public static void spyPlayer(final Player player, final Player other) {
		...
		if (!OldMagicSystem.checkRunes1(player, true, OldMagicSystem.ASTRAL_RUNE, 2, OldMagicSystem.COSMIC_RUNE, 2, OldMagicSystem.BODY_RUNE, 5)) {
			return;
		}
		player.stopAll();
		...
		player.getPackets().sendIComponentText(523, 103,
				"Viewing stats for <br><img=" + other.getIcon() + ">" + other.getDisplayName());
		for (int i = 0; i < 25; i++) {
	...
	public static void SpyNPC(final Player player, final NPC npc) {
		...
		if (!OldMagicSystem.checkRunes1(player, true, OldMagicSystem.COSMIC_RUNE, 1, OldMagicSystem.ASTRAL_RUNE, 1, OldMagicSystem.MIND_RUNE, 1)) {
			return;
		}
		player.stopAll();
	...
	public static final void processDungSpell(Player player, int spellId, int slot, int packetId) {
		...
		case 46: // high alch
			...
			if (!OldMagicSystem.checkSpellLevel(player, (highAlch ? 55 : 21)))
				return;
		...
		case 31:// bones to bananas
			if (!OldMagicSystem.checkSpellLevel(player, 15))
				return;
		...
		case 55:
			...
			break;
		case 57:
			if (!OldMagicSystem.checkSpellLevel(player, 74))
				return;
		...
		default:
			if (Settings.DEBUG)
				Logger.log(OldMagicSystem.class, "Component " + spellId);
			break;
		}
	}
