...
public class ModernMagicks {
	...
	public static final int AIR = 556, WATER = 555, EARTH = 557, FIRE = 554, MIND = 558, NATURE = 561, CHAOS = 562,
			DEATH = 560, BLOOD = 565, SOUL = 566, ASTRAL = 9075, LAW = 563, ARMADYL = 21773, BODY = 559, BANANA = 1963,
			COSMIC = 564;

	public static final int INVENTORY = 0, NPC = 1, PLAYER = 2, OBJECT = 3, ONE_CLICK = 4, COMBAT = 5, FLOOR = 6;
	...
	public enum RSSpellStore {
		...
		// NORMAL SPELLBOOK
		HOME_TELEPORT(24, 0, 0, Settings.HOME, null, ONE_CLICK),
		...
		MOBILISING_TELEPORT(37, 10, 19, new WorldTile(2413, 2848, 0),
				new Item[] { new Item(LAW, 1), new Item(WATER, 1), new Item(AIR, 1) }, ONE_CLICK),
		...
		VARROCK_TELEPORT(40, 25, 35, new WorldTile(3212, 3424, 0),
				new Item[] { new Item(FIRE, 1), new Item(AIR, 3), new Item(LAW, 1) }, ONE_CLICK),
		...
		LUMBRIDGE_TELEPORT(43, 31, 41, new WorldTile(3222, 3218, 0),
				new Item[] { new Item(EARTH, 1), new Item(AIR, 3), new Item(LAW, 1) }, ONE_CLICK),
		...
		FALADOR_TELEPORT(46, 37, 48, new WorldTile(2964, 3379, 0),
				new Item[] { new Item(WATER, 1), new Item(AIR, 3), new Item(LAW, 1) }, ONE_CLICK),

		CRUMBLE_UNDEAD(47, 39, new Item[] { new Item(EARTH, 2), new Item(AIR, 2), new Item(CHAOS, 1) }, COMBAT),

		HOUSE_TELEPORT(48, 40, 25, new WorldTile(3222, 3222, 0),
				new Item[] { new Item(LAW, 1), new Item(AIR, 1), new Item(EARTH, 1) }, ONE_CLICK),
		...
		CAMELOT_TELEPORT(51, 45, 55.5, new WorldTile(2757, 3478, 0), new Item[] { new Item(AIR, 5), new Item(LAW, 1) },
				ONE_CLICK),
		...
		SLAYER_DART(56, 50, new Item[] { new Item(DEATH, 1), new Item(MIND, 4) },
				new Item[] { new Item(4170), new Item(24203) }, COMBAT),

		ARDOUGNE_TELEPORT(57, 51, 61, new WorldTile(2664, 3305, 0), new Item[] { new Item(WATER, 2), new Item(LAW, 2) },
				ONE_CLICK),
		...
		WATCHTOWER_TELEPORT(62, 58, 68, new WorldTile(2547, 3113, 2),
				new Item[] { new Item(EARTH, 2), new Item(LAW, 2) }, ONE_CLICK),
		...
		SARADOMIN_STRIKE(66, 60, new Item[] { new Item(FIRE, 2), new Item(BLOOD, 2), new Item(AIR, 4) },
				new Item[] { new Item(2415) }, COMBAT),

		CLAWS_OF_GUTHIX(67, 60, new Item[] { new Item(FIRE, 1), new Item(BLOOD, 2), new Item(AIR, 4) },
				new Item[] { new Item(2416), new Item(8841) }, COMBAT),

		FLAMES_OF_ZAMORAK(68, 60, new Item[] { new Item(FIRE, 4), new Item(BLOOD, 2), new Item(AIR, 1) },
				new Item[] { new Item(2417) }, COMBAT),

		TROLLHEIM_TELEPORT(69, 61, 68, new WorldTile(2888, 3674, 0), new Item[] { new Item(FIRE, 2), new Item(LAW, 2) },
				ONE_CLICK),
		...
		APE_ATOLL_TELEPORT(72, 64, 76, new WorldTile(2762, 9094, 0),
				new Item[] { new Item(FIRE, 2), new Item(WATER, 2), new Item(LAW, 2), new Item(BANANA, 1) }, ONE_CLICK),
		...
		TELEOTHER_LUMBRIDGE(79, 74, new Item[] { new Item(SOUL, 1), new Item(LAW, 1), new Item(EARTH, 1) }, PLAYER),
		...
		TELEOTHER_FALADOR(85, 82, new Item[] { new Item(SOUL, 1), new Item(WATER, 1), new Item(LAW, 1) }, PLAYER),

		TELEPORT_BLOCK(86, 85, new Item[] { new Item(CHAOS, 1), new Item(LAW, 1), new Item(DEATH, 1) }, COMBAT),

		WATER_SURGE(87, 85,
				new Item[] { new Item(WATER, 10), new Item(AIR, 7), new Item(BLOOD, 1), new Item(DEATH, 1) }, COMBAT),

		ENCHANT6(88, 87, new Item[] { new Item(EARTH, 20), new Item(FIRE, 20), new Item(COSMIC, 1) }, INVENTORY),

		EARTH_SURGE(89, 90,
				new Item[] { new Item(EARTH, 10), new Item(AIR, 7), new Item(BLOOD, 1), new Item(DEATH, 1) }, COMBAT),

		TELEOTHER_CAMELOT(90, 90, new Item[] { new Item(SOUL, 2), new Item(LAW, 1) }, PLAYER),

		FIRE_SURGE(91, 95, new Item[] { new Item(FIRE, 10), new Item(AIR, 10), new Item(BLOOD, 1), new Item(DEATH, 1) },
				COMBAT),
	...
	public static void handleOneClickSpell(Player player, int spellId) {
		...
		if (name.contains("enchant_bolt")) {
			player.stopAll();
			player.getInterfaceManager().sendInterface(432);
			player.getPackets().sendItemOnIComponent(432, 17, 9236, 5);
		...
		} else if (name.contains("bones_to_")) {
			if (BonesTo.cast(player, s.getXp(), name.contains("bones_to_peaches")))
				removeRunes(player, spellId);
		} else if (name.contains("charge")) {
			if (Charge.castSpell(player))
				removeRunes(player, spellId);
			
		} else if (s.name().toLowerCase().contains("home_teleport")) {
	...
	public static void handleInventorySpell(Player player, int spellId, int itemId, int slotId) {
		...
		if (name.contains("alchemy")) {
			...
			if (Alchemy.castSpell(player, itemId, slotId, fireStaff, s.name().toLowerCase().contains("low_")))
				removeRunes(player, spellId);
		} else if (name.contains("superheat")) {
			if (SuperHeat.cast(player, s.getXp(), itemId, slotId))
				removeRunes(player, spellId);
		} else if (name.contains("enchant1")) {
			if (Enchanting.startEnchant(player, itemId, slotId, 1, s.getXp()))
				removeRunes(player, spellId);
		} else if (name.contains("enchant2")) {
			if (Enchanting.startEnchant(player, itemId, slotId, 2, s.getXp()))
				removeRunes(player, spellId);
		} else if (name.contains("enchant3")) {
			if (Enchanting.startEnchant(player, itemId, slotId, 3, s.getXp()))
				removeRunes(player, spellId);
		} else if (name.contains("enchant4")) {
			if (Enchanting.startEnchant(player, itemId, slotId, 4, s.getXp()))
				removeRunes(player, spellId);
		} else if (name.contains("enchant5")) {
			if (Enchanting.startEnchant(player, itemId, slotId, 5, s.getXp()))
				removeRunes(player, spellId);
		} else if (name.contains("enchant6")) {
			if (Enchanting.startEnchant(player, itemId, slotId, 6, s.getXp()))
				removeRunes(player, spellId);
		}
	}
	...
	public static void handleObjectSpell(Player player, int spellId, int objectId) {
		...
		if (name.contains("air_orb") && objectId == 2152) {
			ChargeOrb.charge(player, 573);
			return;
		} else if (name.contains("water_orb") && objectId == 2151) {
			ChargeOrb.charge(player, 571);
			return;
		} else if (name.contains("earth_orb")) {
			ChargeOrb.charge(player, 575);
			return;
		} else if (name.contains("fire_orb") && objectId == 2153) {
	...
	public static void handleFloorSpell(Player player, int spellId, FloorItem floorItem) {
		...
		if (s == null)// Check is done here already
			return;
		String name = s.name().toLowerCase();
		player.getPackets().sendGameMessage("Telegrab here");
		// TODO TELEGRAB, GET PACKET
		if (name.contains("")) {
		}
	}
	...
	public static void handleTargetSpell(Player player, int spellId, Entity target) {
		...
		String name = s.name().toLowerCase();
		// TODO TELEOTHER SPELLS
		if (name.contains("teleother")) {
			player.getPackets()
					.sendGameMessage("Teleport " + p2.getDisplayName() + " to " + name.replace("teleother", ""));
		}
	}

	public static final void sendNormalTeleportSpell(Player player, double xp, WorldTile tile) {
