...
public class GodWarsDoor extends Dialogue {
	...
	public void start() {
		try {
			...
			sendOptionsDialogue(TITLE, (player.isLegendaryMember() ? "Free" : player.isExtremeMember() ? "Pay 75,00 coins" : player.isSuperMember() ? "Pay 150,00 coins." : player.isMember() ? "Pay 250,000 coins." : "Pay 500,00 coins."), "Nothing.");
	...
	public void run(int interfaceId, int componentId) {
		if (stage == -1) {
			...
			case OPTION_1:
				boolean paid = false;
				int amount = player.isLegendaryMember() ? 0 : player.isExtremeMember() ? 75000 : player.isSuperMember() ? 150000 : player.isMember() ? 250000 : 500000;
