	...
	public void checkRequirement(int id){
		...
		for (AchievementKills achievement : AchievementKills.values()) {
			if (achievement.getId() == id) {
				...
				totalKills = (player.getBossKillcount().get(name) != null ? player.getBossKillcount().get(name).intValue() : 0);
				if (totalKills < achievement.getKills()) {
					player.sm("You must have killed at least " + achievement.getKills() + " " + name + ", "
