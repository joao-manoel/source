...
public class Appearence implements Serializable {
	...
	public void generateAppearenceData() {
		...
		if (title != 0) {
			if (title == 65535 && !player.getIronman().isIronman())
				title = 0;
			setTitleName(title == 25 ? "<col=C12006>Yt'Haar </col>"
					: title == 300 ? "<col=f70f0f>Legendary </col>"
							: title == 301 ? "<col=6e200d>Extreme </col>"
									: title == 302 ? "<col=1021e0>Super </col>"
											: title == 303 ? "<col=18c446>Member </col>"
					: title == 5000 ? "<col=C12006>" + player.clanTag + "</col>"
							: title == 9001 ? "<col=734d26> the Recruiter"
									: title == 9002 ? "<col=827e7d> the Recruiter"
											...
											: title == 9004 ? "<col=aa44aa> Marechal</col> "
													: title == 9005 ?"<col=c603fc> The Dice "
															: title == 9006 ?"<col=fcba03>"
																	: title == 9007 ?"<col=f71105>"
													: title == 800 ? "<col="+player.getCustomTitle() + ">" : title == 900 ? player.getCustomTitle()
															: title == 901 ? player.getCustomTitle2()
																	: title == 65535 ? player.getIronman().getTitle()
		...
		player.getPackets().sendIComponentText(884, 1, "Combat Lvl: " + (!pvpArea || summoningLevels == 0 ? player.getSkills().getCombatLevelWithSummoning() : player.getSkills().getCombatLevel() + "+" + summoningLevels));
		stream.writeByte(-1);
	...
	public void setHidden(boolean hidden) {
		hidePlayer = hidden;
		player.getAppearence().transformIntoNPC(hidden ? 82882 : -1);
		generateAppearenceData();
	}
}
