...
public class Player extends Entity {
	...
	private boolean trimedCape = false;
	
	public boolean getTrimedCape() {
		return trimedCape;
	}
	public void setTrimedCape(boolean value) {
		this.trimedCape = value;
	}
	...
	public int setGraveStone(int gs) {
		return graveStone = gs;
	}
	 ...
	 * @AvalonPoints
	 */
	private int ressurgePoints;
	
	private int rubyPoints;
	...
	public PkPointsManager PkPointsManager;
	...
	public Player(String password) {
		...
		resetBarrows();
		lodestone = new boolean[] { true, true, true, true, true, true, true, true, true, true, true, true,
				true, true, true };
	...
	public static boolean isBeta(Player player) {
		for (String name : Settings.BETAS) {
	...
	public double getBonusPoints() {
		...
		if(isLegendaryMember()) {
			points += 5.0;
		}else if(isExtremeMember()) {
			points += 4.0;
		}else if(isSuperMember()) {
			points += 3.0;
		}else if (isMember()) {
			points += 2.0;
		}
	...
	public void checkMaxCape(Player player) {
		int MAX_HOOD = 20768, MAX_CAPE = 20767;
		if(!player.hasMaxCapeRequirements()) {
			player.lock(1);
			Dialogue.sendItemDialogueNoContinue(player, MAX_CAPE, 1,
			...
			if (player.getEquipment().getHatId() == MAX_HOOD) {
				player.getEquipment().deleteItem(MAX_HOOD, 1);
				if (player.getInventory().hasFreeSlots())
					player.getInventory().addItem(MAX_HOOD, 1);
				else
					player.getBank().addItem(MAX_HOOD, 1, true);
			}
			if (player.getEquipment().getCapeId() == MAX_CAPE) {
				player.getEquipment().deleteItem(MAX_CAPE, 1);
				if (player.getInventory().hasFreeSlots())
					player.getInventory().addItem(MAX_CAPE, 1);
				else
					player.getBank().addItem(MAX_CAPE, 1, true);
			}
		}
	...
	public boolean hasCompletionistRequirements() {
		if (getSkills().getLevelForXp(Skills.ATTACK) >= 99 && getSkills().getLevelForXp(Skills.STRENGTH) >= 99
				...
				&& checkKillsInt("Corporeal Beast") >= 10 && checkKillsInt("General Graardor") >= 25
	...
	public void sendCompTrimmedReqMessages() {
		if (!hasTrimCompReqs()) {
		...
		if(getSlayerManager().getCompletedTasks() < 50)
			sm("You must have completed 50 slayer task.");
		if(!getTaskManager().hasCompletedAllTasks())
			sm("You need to complete all tasks.");
		if(checkKillsInt("King Black Dragon") < 100)
			sm("You must have killed at least 100 King Black Dragon, " + (100 - checkKillsInt("King Black Dragon")) + " left.");
		if(checkKillsInt("Corporeal Beast") < 100)
			sm("You must have killed at least 100 Corporeal Beast, " + (100 - checkKillsInt("Corporeal Beast")) + " left.");
		if(checkKillsInt("General Graardor") < 100)
			sm("You must have killed at least 100 General Graardor, " + (100 - checkKillsInt("General Graardor")) + " left.");
		if(checkKillsInt("Commander Zilyana") < 100)
			sm("You must have killed at least 100 Commander Zilyana, " + (100 - checkKillsInt("Commander Zilyana")) + " left.");
		if(checkKillsInt("K'ril Tsutsaroth") < 100)
			sm("You must have killed at least 100 K'ril Tsutsaroth, " + (100 - checkKillsInt("K'ril Tsutsaroth")) + " left.");
		if(checkKillsInt("Kree'arra") < 100)
			sm("You must have killed at least 100 Kree'arra, " + (100 - checkKillsInt("Kree'arra")) + " left.");
		if(checkKillsInt("Dagannoth Rex") < 100)
			sm("You must have killed at least 100 Dagannoth Rex, " + (100 - checkKillsInt("Dagannoth Rex")) + " left.");
		if(checkKillsInt("Dagannoth Prime") < 100)
			sm("You must have killed at least 100 Dagannoth Prime, " + (100 - checkKillsInt("Dagannoth Prime")) + " left.");
		if(checkKillsInt("Dagannoth Supreme") < 100)
			sm("You must have killed at least 100 Dagannoth Supreme, " + (100 - checkKillsInt("Dagannoth Supreme")) + " left.");
		if(checkKillsInt("Nex") < 100)
			sm("You must have killed at least 100 Nex, " + (100 - checkKillsInt("Nex")) + " left.");
	}
	...
	public boolean hasTrimCompReqs() {
		if (getSkills().getLevelForXp(Skills.ATTACK) >= 99 && getSkills().getLevelForXp(Skills.STRENGTH) >= 99
				...
				&& getSlayerManager().getCompletedTasks() >= 50
				&& isCompletedFightCaves() 
				&& checkKillsInt("King Black Dragon") >= 100
				&& checkKillsInt("Corporeal Beast") >= 100 
				&& checkKillsInt("General Graardor") >= 100
				&& checkKillsInt("Commander Zilyana") >= 100 
				&& checkKillsInt("K'ril Tsutsaroth") >= 100
				&& checkKillsInt("Kree'arra") >= 100
				&& checkKillsInt("Dagannoth Rex") >= 100
				&& checkKillsInt("Dagannoth Prime") >= 100 
				&& checkKillsInt("Dagannoth Supreme") >= 100
				&& checkKillsInt("Nex") >= 100
				&& getTaskManager().hasCompletedAllTasks())
			return true;
		return false;
	}
	...
	public void sendPlayersList() {
		...
		for (Player p5 : World.getPlayers()) {
			...
			if(p5.isLegendaryMember()) {
				titles = "<col=f70f0f><shad=2E2EFE>Legendary Member</shad></col> <img=22> ";//icones
			}
			else if(p5.isExtremeMember()) {
				titles = "<shad=2E2EFE><col=6e200d>Extreme Member</col></shad> <img=11> ";//icones
			}
			else if(p5.isSuperMember()) {
				titles = "<shad=2E2EFE><col=1021e0>Super Member</col></shad> <img=10> ";//icones
			}
			else if(p5.isMember()) {
				titles = "<shad=2E2EFE><col=18c446>Member</col></shad> <img=9> ";//icones
			}
			...
			if (p5.getRights() == 2) {
				titles = "<col=00ffff><shad=0000ff>Administrador</col></shad> <img=1> ";
			}
			if (p5.getRights() == 0 && p5.isSupporter()) {
				titles = "<col=58ACFA><shad=2E2EFE>" + Settings.SERVER_NAME + " Support</shad></col> <img=20> ";
			}
			
			if (p5.getUsername().equalsIgnoreCase("manoel")) {
				titles = "<col=ff0000><shad=000000>Owner</col></shad> <img=1> ";
			}
	...
	public void processEntity() {
		...
		if (!(getControlerManager().getControler() instanceof WildernessControler) && isAtWild()
				&& !WildernessControler.isAtWild(this) && !isAtPvP()) {
	...
	public void run() {
		...
		//getPackets().sendGameMessage("<col=1f92f0>"+Settings.LASTEST_UPDATE+"</col>");
		//getPackets().sendGameMessage("<col=eb4034>Dia 10/07/2020 vai ocorrer uma manuten��o no servidor, podendo fica ate 24hr fora do ar, fique atento ao discord/site para saber quando vai voltar.</col>");
	...
	private void checkRights() {
		...
		for (String developers : Settings.SUPPORT) {
			if (getUsername().equalsIgnoreCase(developers)) {
	...
	public PkPointsManager getPkPointsShop() {
		return  PkPointsManager;
	}
	...
	public void handleWrath(final Entity source) {
	    ...
	    public void sendItemsOnDeath(Player killer, WorldTile deathTile, WorldTile respawnTile, boolean wilderness, Integer[][] slots) {
			...
			for (int i = 0; i < items[0].length; i++) {
				...
				if (ItemConstants.keptOnDeath(item)) {
					if (item.getId() == 24497 && isAtWild()) {
						...
						World.updateGroundItem(new Item(995, item.getDefinitions().getTipitPrice() * item.getAmount()), deathTile, killer == null ? this : killer, 60, 1,
								killer.getIronman().isIronman() ? killer.getDisplayName() : null);
			...
			for (int i = 0; i < items[1].length; i++) {
				...
				if (item.getId() == 11283) {
					items[1][i] = new Item(11284);
					setDfsCharges(0);
				}
				if (!ItemConstants.keptOnDeath(item) && !ItemConstants.isTradeable(item))
			...
			if (killer != this) {
				int randomCoins = Utils.random(1, 1000);
				randomCoins += killer.totalCurrentDrop;
	...
	public boolean asfriendrefer;
	
	public boolean getAsFriendRefer() {
		return asfriendrefer;
	}
	public void setAsfriendRefer() {
		this.asfriendrefer = true;
	}
	...
	public void setSuperMember(boolean i) {
		this.supermember = i;
	}
	
	
	public boolean legendarymember;
	
	public boolean isLegendaryMember() {
		return legendarymember;
	}
	
	public void setLegendaryMember(boolean i) {
		this.legendarymember = i;
	}
	
	public boolean extrememember;
	
	public boolean isExtremeMember() {
		return extrememember;
	}
	
	public void setExtremeMember(boolean i) {
		this.extrememember = i;
	}
	...
	public void sendCompReqMessages() {
		...
		checkRequirement(6222);
		checkRequirement(6203);
		...
		checkRequirement("Nex");
	}
	
	
	public void checkRequirement(int id){
		int totalKills = 0;
		String name = null;
	...
	public void checkRequirement(String name) {
		int totalKills = 0;
		for (AchievementKills achievement : AchievementKills.values()) {
			if (achievement.name().replace("_", " ").equalsIgnoreCase(name)) {
				totalKills = (getBossKillcount().get(name) != null ? getBossKillcount().get(name).intValue() : 0);
				if (totalKills < achievement.getKills()) {
					sm("You must have killed at least " + achievement.getKills() + " " + name + ", "
							+ (achievement.getKills() - totalKills) + " left.");
	...
	public int getRessurgePoints() {
		return ressurgePoints;
	}

	public void setRessurgePoints(int ressurgePoints) {
		this.ressurgePoints = ressurgePoints;
	}
	...
	public boolean setHasGloves(boolean i) {
		return this.hasGloves = i;
	}
	
	public int getRubyPoints() {
		return rubyPoints;
	}
	
	public void setRubyPoints(int rubyPoints) {
		this.rubyPoints = rubyPoints;
	}
	...
	public int mainrspoints;
	public int getmainrspoints() {
		return mainrspoints;
	}
	...
	public int getDungeonToken() {
		return dungeonToken;
	}
	
	public void setDungeonToken(int token) {
		this.dungeonToken = token;
	}
...
private transient VarsManager VBM;
private transient LividFarm LIVIDFARM;
private boolean shopBlockMode;
	...
	public int explorerRing1;

	public boolean setBlueBuy(boolean i) {
		return this.isBlueBuy = i;
	}
	
	public boolean getBlueBuy() {
		return this.isBlueBuy;
	}
	...
	public VarsManager getVarBitManager() {
		return VBM;
	}
	
	public void refreshVerboseShopDisplayMode() {
    	VBM.sendVarBit(11055, shopBlockMode ? 0 : 1);
    }
	...
	public Animation getDeathAnimation() {
		gfx(new Graphics(Utils.random(2) == 0 ? 4399 : 4398));
