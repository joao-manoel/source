...
public class PkPointsManager {
	...
	private static ItemsContainer<Item> pkpItems;
	...
	public void refreshItems(Item[] itemsBefore) {
		...
		for (int index = 0; index < itemsBefore.length; index++) {
			Item item = pkpItems.getItems()[index];
			if (itemsBefore[index] != item) {
				...
				for (PkPointStore store : PkPointStore.values()) {
					if (store != null) {
						...
						// dont think wat ur doing is possible because config is connected to var
						// scripts notits works on price checker tho? its the same
					}
				}
			}
