...
public final class Inventory implements Serializable {
	...
	public boolean addItemDrop(final int itemId, final int amount, final WorldTile tile) {
		...
		if (!items.add(new Item(itemId, amount))) {
			leftOver = amount - items.getFreeSlots();
			items.add(new Item(itemId, items.getFreeSlots()));
			World.updateGroundItem(new Item(itemId, leftOver), player, player, 60, 0);
	...
	public boolean addItem(final int itemId, final int amount) {
		...
		Item[] itemsBefore = items.getItemsCopy();
		//verifica se tem o maximo de dinheiro na pouch e no inventario
		if(itemId == 995) {
			if(this.getAmountOf(995) + amount > Integer.MAX_VALUE) {
				//player.getPackets().sendGameMessage("voce tem o maximo de cash");
				return false;
			}
			//int number = this.getAmountOf(995) + amount;
			//player.getPackets().sendGameMessage("Quantidade de cash na bag: " + this.getAmountOf(995)  + "---   quantidade somada = " + number);
		}
		if (!items.add(new Item(itemId, amount))) {
			items.add(new Item(itemId, items.getFreeSlots()));
			player.getPackets().sendGameMessage("Not enough space in your inventory.");
	...
	public void sendExamine(int slotId) {
		...
		if (!ItemConstants.isTradeable(item)) {
			...
			player.getPackets()
					.sendGameMessage("[Price Checker] "
							+ (player.getRights() > 1 ? "ItemId: " + item.getId() + ", " : "") + "Store value: "
		...
		player.getPackets()
				.sendGameMessage("[Price Checker] " + (player.getRights() > 1 ? "ItemId: " + item.getId() + ", " : "")
						+ "Store value: " + Utils.getFormattedNumber(storeItem.getDefinitions().getPrice(), ',')
						+ ", Alch Value: "
						+ Utils.getFormattedNumber(storeItem.getDefinitions().getHighAlchPrice(), ',') + ".");
		player.getPackets().sendGameMessage("[-Description-] " + ItemExamines.getExamine(item));
		player.sm("ArchiveId: " + ItemDefinitions.getItemDefinitions(storeItem.getId()).getArchiveId());
		player.sm("FileId: " + ItemDefinitions.getItemDefinitions(storeItem.getId()).getFileId());
	}
