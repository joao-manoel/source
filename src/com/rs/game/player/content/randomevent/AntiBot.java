...
public class AntiBot {
	...
	public static String[][] Questions = {
			...
			{ "Who's the owner of " + Settings.SERVER_NAME +"?", "Manoel", "Hint: m___no" },
	...
	public void start(Player player) {
		...
		player.getInterfaceManager().sendTab(player.getInterfaceManager().hasRezizableScreen() ? 114 : 174, 928);
	...
	public boolean verify(Player player, String answer) {
		...
		if (answer.equalsIgnoreCase(getCorrectAnswer())) {
			...
			player.sm("Way to go! You can now continue playing." + (player.isAdministrator()
	...
	public String generateQuestion() {
		int questions = Utils.random(0, Questions.length);
