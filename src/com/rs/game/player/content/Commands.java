...
import com.rs.game.player.controlers.lividfarm.LividFarmControler;
...
public final class Commands {
	...
	public static void commandsMessage(Player player) {
		player.getInterfaceManager().sendInterface(275);
		player.getPackets().sendIComponentText(275, 1, "*Commands*");
		player.getPackets().sendIComponentText(275, 10, "");
		player.getPackets().sendIComponentText(275, 11, "::ticket");
		player.getPackets().sendIComponentText(275, 12, "::train2 - Stronghold of Security");
		player.getPackets().sendIComponentText(275, 13, "::train - Rock crabs");
		player.getPackets().sendIComponentText(275, 14, "::az - armoured zombie");
		player.getPackets().sendIComponentText(275, 15, "::atol");
		player.getPackets().sendIComponentText(275, 16, "::lobster - Rock Lobster");
		player.getPackets().sendIComponentText(275, 17, "::empty - clean you inventory");
		player.getPackets().sendIComponentText(275, 18, "::geoffers - show all offers");
		...
		player.getPackets().sendIComponentText(275, 26, "::switchitemslook - Changes items look from old / new");
		player.getPackets().sendIComponentText(275, 27, "::compreqs - Shows completionist cape requirements");
		player.getPackets().sendIComponentText(275, 28, "::comptrimmedreqs - Shows completionist trimmed cape requirements");
	...
	public static boolean processNormalCommand(final Player player, String[] cmd, boolean console,
		...
		if (clientCommand) {
			...
			//teles start				
			case "train":
				Magic.sendNormalTeleportSpell(player, 0, 0.0D, new WorldTile(2703, 3716, 0), new int[0]);
				return true;
				
				
			case "train2":
				Magic.sendNormalTeleportSpell(player, 0, 0.0D, new WorldTile(3082, 3423, 0), new int[0]);
				return true;
			...
			case "comptrimmedreqs":
				player.sendCompTrimmedReqMessages();
				return true;
				
			case "diceme":
				if(player.isDicer()) {
					if(hasDiceBag(player)) {
						player.getInventory().addItem(new Item(15098, 1));
						player.sm("You received a dice bag");
					}
					player.animate(new Animation(787));
					player.getAppearence().setTitle(9005);
					
				}else {
					player.sm("only dicers.");
				}
				return true;
				
			case "atol":
				 Magic.sendNormalTeleportSpell(player, 0, 0.0D, new WorldTile(2759, 9093, 0),
	                        new int[0]);
	                return true;
	                
			case "lobster":
				 Magic.sendNormalTeleportSpell(player, 0, 0.0D, new WorldTile(1892, 4409, 0),
	                        new int[0]);
	                return true;
				
			case "az":
                Magic.sendNormalTeleportSpell(player, 0, 0.0D, new WorldTile(3242, 9996, 0),
                        new int[0]);
                return true;
				
			case "blueskin":
				if(player.isMember() && player.getBlueBuy()) {
			...
			case "puropuro":
				Magic.sendNormalTeleportSpell(player, 0, 0.0D, new WorldTile(2421, 4444, 0),new int[0]);
				return true;
			
			case "glacors":
				  Magic.sendNormalTeleportSpell(player, 0, 0.0D, new WorldTile(4180, 5726, 0),new int[0]);
				  return true;
			...
			case "changedisplay":
				if (!player.isMember()) {
					...
					player.sm("contact administrador change username ;;ticket");
				}
				return true;
			case "changepass":
				String message = "";
			...
			case "ice":
				player.setNextWorldTile(new WorldTile(3432, 5653, 0));
				return true;
			
			case "nex":
                Magic.sendNormalTeleportSpell(player, 0, 0.0D, new WorldTile(2903, 5204, 0),
                        new int[0]);
                return true;
			...
			case "getcapeid":
				...
				if(skillname == null && option == null) {
					player.sm("You can`t use ::getcapeid skillname 0 or 1, [0 = untrimed] - [1 = trimed]");					
				}else {
					int getCapeId = SkillCapes.getCapeBySkillName(skillname, option.contains("1") ? true : false);
					 String getNameCape = ItemDefinitions.getItemDefinitions(getCapeId).getName();
					if(getCapeId > 0) {
						player.sm("You id of "+getNameCape+" is ("+getCapeId+")." );
					}
	...
	public static boolean hasDiceBag(Player player) {
		if(!player.getInventory().contains(new Item(15084)) 
				&& !player.getInventory().contains(new Item(15085))
				&& !player.getInventory().contains(new Item(15088))
				&& !player.getInventory().contains(new Item(15089))
				&& !player.getInventory().contains(new Item(15098))
				&& !player.getInventory().contains(new Item(15099))
				&& !player.getInventory().contains(new Item(24869))
				&& !player.getInventory().contains(new Item(26241))
				/*
				//bank				
				&& !player.getBank().containsOneItem(15084)
				&& !player.getBank().containsOneItem(15085)
				&& !player.getBank().containsOneItem(15088)
				&& !player.getBank().containsOneItem(15089)
				&& !player.getBank().containsOneItem(15098)
				&& !player.getBank().containsOneItem(15099)
				&& !player.getBank().containsOneItem(24869)
				&& !player.getBank().containsOneItem(26241)
				*/
				) {
			return true;
	...
	public static boolean processAdminCommands(final Player player, String[] cmd, boolean console,
		...
		if (clientCommand) {
			...
			case "updatecount":
				try {
					Launcher.setWebsitePlayersOnline(World.getPlayers().size());
					player.sm("Site foi atualizado tem: " +World.getPlayers().size() + " player online.");
			...
			case "rotten":
				if(isDeveloper(player)) {
					player.getInventory().addItem(new Item(5733, 1));
					return true;
				}
				return true;
			/*
			...
			case "setdice":
				...
				if (cmd.length >= 1) {
					...
					p.setDicer(true);
					if(hasDiceBag(p)) {
						p.getInventory().addItem(new Item(15084, 1));
						p.sm("You received a dice bag");
						p.sm("<col=fcba03>I'll WIN</col>");
					}
					player.animate(new Animation(787));
					p.getAppearence().setTitle(9005);
					p.sm("You received the dicer rank");
					player.sm(p.getUsername() + " received rank dicer.");
					return true;
				}
				player.getPackets().sendGameMessage("Use: ::setdice username");
				return true;
			...
			case "removedice":
				...
				if (cmd.length == 2) {
					Player p = World.getPlayerByDisplayName(Utils.formatPlayerNameForDisplay(cmd[1]));
					p.setDicer(false);
					p.getAppearence().setTitle(0);
					p.sm("removed the dicer rank");
					player.sm(p.getUsername() + " removed rank dicer.");
					return true;
				}
				player.getPackets().sendGameMessage("Use: ::removedice username");
				return true;
			...
			case "design":
				CharacterDesign.openPlayerCustomizing(player);
				return true;
			...
			case "raredrop":
				...
				return true;
			case "rmsupport":
				Player p1 = World.getPlayerByDisplayName(Utils.formatPlayerNameForDisplay(cmd[1]));
				p1.setSupporter(false);
				break;
			case "rmmod":
				Player p2 = World.getPlayerByDisplayName(Utils.formatPlayerNameForDisplay(cmd[1]));
				p2.setRights(0);
				break;
			case "give":
			...
			case "getslayerpoints":
				player.getSlayerManager().setPoints(1000);
				return true;
			case "resettask":
			...
			case "igive":
				if (!isDeveloper(player)) {
	             ...
	             String itemname = cmd[1].substring(cmd[1].indexOf(" ") + 1);
	             Player other70 = World.getPlayerByDisplayName(itemname);
	             if (other70 == null) {
	                  return true;
	              }
	              ...
	              if (cmd.length < 2) {
	               ...
	               try {
	                    other70.getInventory().addItem(
	                            itemId,
	                            amountId);
	                    other70.getPackets().sendGameMessage("You have recieved something in your inventory");
	                    player.getPackets().sendGameMessage("You have given something to your friend.");
	                } catch (NumberFormatException e) {
	                    player.getPackets().sendGameMessage(
			...
			case "setruby":
				...
				if (cmd.length == 2 || cmd.length == 3) {
					...
					if (p != null) {
						try {
							p.setRubyPoints(p.getRubyPoints() + amount);
							p.getPackets().sendGameMessage("You recieve " + amount + " ruby points.");
							player.getPackets().sendGameMessage("You have given "+amount+" ruby points for "+ p.getUsername());
							return true;
						} catch (NumberFormatException e) {
						}
					}
				}
				player.getPackets().sendGameMessage("Use: ::setruby username amount (optional:amount)");
				return true;
			case "removeruby":
				if (!isDeveloper(player))
					return true;
			...
			case "makelegendary":
				if (cmd.length == 1) {
					...
					if (p != null) {
						try {
							p.setLegendaryMember(true);
							p.getPackets().sendGameMessage("You recieve Extreme Member rank.");
							player.getPackets().sendGameMessage("You given extreme member rank for "+ p.getUsername() +".");
							return true;
						}catch (NumberFormatException e) {
						}
					}
				}
				player.getPackets().sendGameMessage("Use: ::makelegendary username");
				return true;	
			...
			case "rmlegendary":
				if (cmd.length == 1) {
					...
					if (p != null) {
						try {
							p.setLegendaryMember(false);
							p.getPackets().sendGameMessage("Removed you Extreme Member rank.");
							player.getPackets().sendGameMessage("Removed extreme member rank for "+ p.getUsername() +".");
							return true;
						}catch (NumberFormatException e) {
						}
					}
				}
				player.getPackets().sendGameMessage("Use: ::makesuper username");
				return true;
			...
			case "makesuper":
				if (cmd.length == 1) {
					...
					if (p != null) {
						try {
							p.setSuperMember(true);
							p.getPackets().sendGameMessage("You recieve Super Member rank.");
							player.getPackets().sendGameMessage("You given super member rank for "+ p.getUsername() +".");
							return true;
						}catch (NumberFormatException e) {
						}
					}
			...
			case "rmsuper":
				if (cmd.length == 1) {
					...
					if (p != null) {
						try {
							p.setSuperMember(false);
							p.getPackets().sendGameMessage("Removed Super Member rank.");
							player.getPackets().sendGameMessage("removed super member rank for "+ p.getUsername() +".");
							return true;
						}catch (NumberFormatException e) {
						}
					}
				...
				if (cmd.length == 2 || cmd.length == 3) {
					...
					if (p != null) {
						try {
							...
							player.getPackets().sendGameMessage("You given " + amount + " days of member for "+ p.getUsername() +".");
							return true;
						} catch (NumberFormatException e) {
						}
					}
			...
			case "checkmember":
				if(player.isMember())
					player.sm("Your membership will expire on " + player.getMemberTill());
				
				player.sm("membership expired.");
				return true;
			case "giveitem":
				if (cmd.length == 3 || cmd.length == 4) {
					Player p = World.getPlayerByDisplayName(Utils.formatPlayerNameForDisplay(cmd[1]));
			...
			case "email":
				player.setEmail(cmd[1]);
				return true;
			case "checkemail":
				player.sm(player.getEmail());
				return true;
			case "gravestone":
				GraveStoneSelection.openSelectionInterface(player);
				return true;
			case "setgs":
				player.setGraveStone(1);
				return true;
			case "setgs2":
				player.setGraveStone(12);
				return true;
				...
				if (cmd.length == 2 || cmd.length == 3) {
					...
					if (p != null) {
						try {
							...
							p.setRessurgePoints(p.getRessurgePoints() + amount);
							return true;
						} catch (NumberFormatException e) {
						}
					}
			...
			case "playerlist":
				...
				for (Player p5 : World.getPlayers()) {
					...
					if (p5.getRights() == 2) {
						titles = "<img=1> <col=00ffff><shad=0000ff>Administrador</col></shad> <img=1> ";
					}
					if (p5.getRights() == 0 && p5.isSupporter()) {
						titles = "<col=58ACFA><shad=2E2EFE>" + Settings.SERVER_NAME + " Support</shad></col> <img=12> ";
					}
					if (p5.getUsername().equalsIgnoreCase("Manoel")) {
						titles = "<img=1> <col=ff0000><shad=000000>Owner</col></shad> <img=1> ";
					}
			...
			case "object":
				try {
					...
					World.spawnObject(new WorldObject(Integer.valueOf(cmd[1]), type, 0, player.getX(),
			...
			case "getpts":
				player.setRessurgePoints(1000000);
			...
			case "gfx":
				...
				return true;
			case "cart":
				KeldagrimCartTravel.Travel(player);
				return true;
			case "anim":
				if (cmd.length < 2) {
	...
	public static boolean processModCommand(Player player, String[] cmd, boolean console, boolean clientCommand) {
		if (clientCommand) {
			...
			case "ticket":
				if (player.isLocked() || player.getControlerManager().getControler() != null) {
					player.getPackets().sendGameMessage("You can not attend here");
					return true;
				}
