...
public class FairyRings {
    ...
    public static enum Rings {
    	...
    	DKQ(new WorldTile(4183, 5726, 0)), //glacors
    ...
    public static boolean checkAll(Player player) {
		if (player.getEquipment().getWeaponId() == 772 || player.getEquipment().getWeaponId() == 9084 || player.getRights() == 2)
		    return true;
    ...
    private static void resetRingHash(Player player) {
		...
		for (int i = 0; i < 3; i++)
			player.getPackets().sendConfig(2341 + i, 0);
    }
    ...
    public static void handleDialButtons(final Player player, int componentId) {
		...
		for (int i = 0; i < 3; i++)
			player.getPackets().sendConfig(2341 + i, locationArray[i] == 1 ? 3 : locationArray[i] == 3 ? 1 : locationArray[i]);
