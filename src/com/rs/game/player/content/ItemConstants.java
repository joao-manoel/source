...
public class ItemConstants {
	...
	public static boolean isTradeable(Item item) {
		...
		if (defs.contains("(deg)"))
			return false;
		if (defs.contains("ring of wealth"))
			return false;
		...
		if (item.getDefinitions().getName().toLowerCase().contains("spellcast"))
			return false;
		if (item.getDefinitions().getName().toLowerCase().contains("TokHaar-kal"))
			return false;
		...
		if (item.getDefinitions().getName().toLowerCase().contains("ring of slaying"))
			return false;
		if (item.getDefinitions().getName().toLowerCase().contains("TokHaar"))
			return false;
		if (item.getDefinitions().getName().toLowerCase().contains("void"))
			return false;
		if (item.getId() == 19888)
