...
public class JournalTab extends CustomTab {
	...
	public enum JournalStore {
		...
		RUBYSTORE(8) {
			...
			@Override
			public void usage(Player p) {
				p.getCustomStore().sendInterface(p, 0, CustomStoreData.RUBY);
			}
		...
		RESSURGEPOINTS(9){
			@Override
			public String text(Player p) {
				return "Ressurge Store";
			}

			@Override
			public void usage(Player p) {
				p.getCustomStore().sendInterface(p, 0, CustomStoreData.AVALON_POINTS);
			}
		},
		...
		PLAYERRANK(11) {
			@Override
			public void usage(Player p) {
				p.getPackets().sendGameMessage("My rank is: " + "<img=" + (p.getRights() == 2 ? 1 : p.getRights() == 1 ? 0 : p.getMessageIcon()) + ">" + (p.getIronman().isIronman() && p.getIronman().Ironman_Mode == 2 ? "Hardcore Ironman" : p.getIronman().isIronman() && p.getIronman().Ironman_Mode == 1 ? "Ironman" : p.getRights() == 2 ? "Developer" : p.getRights() == 1 ? "Player Moderator" : p.getPlayerMode() == 5 ? "PK King" : p.getPlayerMode() == 4 ? "Youtuber" : p.getPlayerMode() == 3 ? "Gold Member" : p.getPlayerMode() == 2 ? "Silver Member" : p.getPlayerMode() == 1 ? "Bronze Member" : p.isLegendaryMember() ? "Legendary" : p.isExtremeMember() ? "Extreme" : p.isSuperMember() ? "Super Member" : p.isMember() ? "Member" :"Player") + ".");
				p.getAppearence().setTitle(p.isLegendaryMember() ? 300 : p.isExtremeMember() ? 301 : p.isSuperMember() ? 302 : p.isMember() ? 303 : -1);
			}

			@Override
			public String text(Player p) {
				return "Rank: " + "<img=" + (p.getRights() == 2 ? 1 : p.getRights() == 1 ? 0 : p.getMessageIcon()) + "><col=04BB3B>" + ( p.getIronman().isIronman() && p.getIronman().Ironman_Mode == 2 ? ("Hardcore Iron" + (p.getAppearence().isMale() ? "man" : "woman")) : p.getIronman().isIronman() && p.getIronman().Ironman_Mode == 1 ? "Ironman" : p.getRights() == 2 ? "Developer" : p.getRights() == 1 ? "Moderator" : p.getPlayerMode() == 5 ? "PK King" : p.getPlayerMode() == 4 ? "Youtuber" : p.getPlayerMode() == 3 ? "Gold Member" : p.getPlayerMode() == 2 ? "Silver Member" : p.getPlayerMode() == 1 ? "Bronze Member" :  p.isLegendaryMember() ? "Legendary" : p.isExtremeMember() ? "Extreme" : p.isSuperMember() ? "Super Member" : p.isMember() ? "Member" : "Player");
			}
		},
		PLAYERTITLE(12) {
			@Override
			public void usage(Player p) {
				if(p.isLegendaryMember() || p.isExtremeMember() || p.isSuperMember() || p.isMember()) {
