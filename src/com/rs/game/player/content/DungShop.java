...
public class DungShop {
	...
	public static void openShop(Player player) {
		...
		player.getPackets().sendIComponentText(940, 31, player.getDungeonToken() + "");
	...
	private static void handleBuy(Player player) {
		...
		if (player.getDungeonToken() >= item.tokens) {
			if (item == DungItem.DUNGEONEERING_EXPERIENCE) {
				...
				player.setDungeonToken(player.getDungeonToken() - item.tokens);
			...
			player.getPackets().sendGameMessage("You do not have enough dungeon token to purchase that item.");
			player.getPackets().sendHideIComponent(940, 42, true);
		}
		player.getPackets().sendIComponentText(940, 31, player.getDungeonToken() + "");
	}
