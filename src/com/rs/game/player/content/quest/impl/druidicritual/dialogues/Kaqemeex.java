...
public class Kaqemeex extends Dialogue {
	...
	public void start() {
		...
		if (qStage == 1) {
			...
			sendOptionsDialogue(DEFAULT, "I believe you can sell me a skillcape of Herblore?");
			stageInt = 101;
		} else {
			switch (stageInt) {
			/*
	...
	public void run(int interfaceId, int componentId) {
		...
		case -1:
			...
			break;
		case 0:
			sendOptionsDialogue(DEFAULT, "Who are you?", "I'm in search of a quest.", "Did you build this?", "I believe you can sell me a skillcape of Herblore?");
			stageInt = 1;
			break;
		case 1:
			switch (componentId) {
			...
			case OPTION_4:
				if(player.getSkills().getLevelForXp(Skills.HERBLORE) >= 99) {
					sendNPCChat(Mood.NORMAL, "Most certainly; it has been a pleasure to watch you grow",
							"as a herbalist. I am privileged to have been instrumental in",
							"you learning, but I must ask for a donation of 99,000",
							"coins to cover the expense of the cape.");
					stageInt = 102;
				}else {
					sendNPCChat(Mood.NORMAL, "Tell me about that cape you're wearing.");
					stageInt = -2;
				}
				break;
			}
		...
		case 103:
			if(componentId == OPTION_1) {
				sendNPCChat(Mood.NORMAL, "Not at all; there are many other adventurers who would"
						+ "love the opportunity to purchase such a prestigious item!"
						+ "you can find me here if you change your mind.");
				stageInt = -2;
			}else if(componentId == OPTION_2) {
				if(player.getInventory().getFreeSlots() >= 2) {
					boolean buyCape = SkillCapes.buySkillCape(player, "herblore", false);
					if(buyCape) {
						sendNPCChat(Mood.HAPPY, "Excellent! Wear that cape with pride my friend.");
						stageInt = -2;
					}else {
						sendNPCChat(Mood.MAD, "But, unfortunately, I was mistaken.");
						stageInt = 104;
					}
				}else {
					sendNPCChat(Mood.NORMAL, "Unfortunately all skillcapes are only available with a free hood,"
			...
			break;
		case 104:
			sendNPCChat(Mood.NORMAL, "Well, come back and see me when you do.");
			stageInt = -2;
			break;
		case -2:
			end();
