...
public class TaskManager implements Serializable {
	...
	public static enum Tasks {
		...
		MINE_ADAMANT_ORE(HARD, 100), MINE_GOLD_ORE(HARD, 500), MINE_COAL(HARD, 500),CHOP_YEW_LOGS(HARD, 100), FISH_SHARK(HARD, 150), CUT_UNCUT_DRAGONSTONE(HARD, 50),
		...
		PURO_PURO(ELITE, 400), FLETCH_MAGIC_LONGBOW(ELITE, 200), CUT_UNCUT_ONYX(ELITE), CHOP_MAGIC_LOGS(ELITE, 150),
		MAKE_EXTREME_STRENGTH(ELITE, 150), SMITH_ADAMANT_PLATEBODY(ELITE, 100), LIGHT_MAGIC_LOG(ELITE, 200),
		GNOME_AGILITY_ADVANCED(ELITE, 25), STEAL_FROM_GEM_STALL(ELITE, 150), KILL_ABYSSAL_DEMON(ELITE, 1000),
		CRAFT_BLOOD_RUNE(ELITE, 500), MINE_RUNITE_ORE(ELITE, 150), SUMMON_UNICORN_STALLION(ELITE, 200), 
	...
	private final String[] finishTaskMessages = { "Nice job!", "Great job!", "Awesome,", "Well done!", "Cool!",
			"Amazing!" };
	...
	public void sendReward(Tasks task, int amount) {
		if (task.getDifficulity() == EASY) {
			player.getMoneyPouch().addMoney(500000, false);
		}
		if (task.getDifficulity() == MEDIUM) {
			player.getMoneyPouch().addMoney(1000000, false);
		}
		if (task.getDifficulity() == HARD) {
			player.getMoneyPouch().addMoney(2500000, false);
		}
		if (task.getDifficulity() == ELITE) {
			player.getMoneyPouch().addMoney(4500000, false);
		}
	}
