...
public class CustomStore extends CustomStoreData {
	...
	public static HashMap<Player, Integer> viewingPlayers = new HashMap<>();
	...
	public void sendItemAndInfo(int container) {
		...
		int totalCurrency = -1;
		if (TYPE != null)
			totalCurrency = SHOP == PK_POINTS ? player.getPKP() : SHOP == RUBY ? player.getRubyPoints()
					: SHOP == AVALON_POINTS ? player.getRessurgePoints()  : player.getTotalCoins();
		if (TYPE == COINS) {
			if (container >= getItems(TYPE, SHOP).size())
				return;
			int price = ( SHOP == PK_POINTS || SHOP == AVALON_POINTS  || SHOP == RUBY ? getItems(TYPE, SHOP).get(container).getAmount() : EconomyPrices.getPrice(getItems(TYPE, SHOP).get(container).getId())
					);
			
			player.getPackets().sendHideIComponent(INTERFACE_ID, COMPONENTS[container], false);// unhides all boxes
			player.getPackets().sendItems(container, new Item[] { new Item(getItems(TYPE, SHOP).get(container).getId(),
					1) });// sends items
			player.getPackets().sendIComponentText(INTERFACE_ID, COMPONENTS[container] + 4,
					getItems(TYPE, SHOP).get(container).getAmount() == 0 ? "Out<br>of<br>stock<br><br><br><br><br>"
							: price == 0 ? HexColours.getShortMessage(Colours.GREEN, "Free")
									: totalCurrency >= price
											? HexColours.getMessage(Colours.YELLOW, formatPrice(price))
			...
			player.getPackets().sendHideIComponent(INTERFACE_ID, COMPONENTS[container] + 3, true);// hides background of
																									// price
			if (SHOP == PK_POINTS || SHOP == AVALON_POINTS || SHOP == RUBY)
		...
		if (TYPE == SKILLCAPE) {
			...
			player.getPackets().sendItems(container,
					new Item[] { new Item( skillcapeId,SkillcapeStore.capes.get(container).getAmount()) });// sends items
	...
	public void sendInfo(int itemId) {
		...
		if (TYPE == SKILLCAPE) {
				...
				if (itemId == item.getId()) {
					...
					player.getPackets()
							.sendGameMessage(ItemDefinitions.getItemDefinitions(itemId).getName()
									+ (price == 0 ? " is " + HexColours.getShortMessage(Colours.GREEN, "free!")
											: (" costs "
			...
			for (Item item : getItems(TYPE, SHOP))
				if (itemId == item.getId()) {
					price = SHOP == PK_POINTS || SHOP == AVALON_POINTS || SHOP == RUBY ? item.getAmount() : EconomyPrices.getPrice(itemId);
					player.getPackets()
							.sendGameMessage(ItemDefinitions.getItemDefinitions(itemId).getName()
									+ (price == 0 ? " is " + HexColours.getShortMessage(Colours.GREEN, "free!")
											: (" costs "
	...
	public void sendBuy(int itemId, int amount) {
		...
		if (TYPE == SKILLCAPE) {
			...
			for (Item item : getItems(TYPE, SHOP)) {
				if (item.getId() == itemId) {
					canBuy = true;
					defaultPrice = SHOP == PK_POINTS || SHOP == AVALON_POINTS || SHOP == RUBY ? item.getAmount() : EconomyPrices.getPrice(itemId);
					points = (SHOP == PK_POINTS ? player.getPKP() : SHOP == AVALON_POINTS ? player.getRessurgePoints() : SHOP == RUBY ? player.getRubyPoints() : -1);
					continue;
				}
			}
		} else {
			for (int i = 0; i < getShopType(TYPE).length; i++)
				if (itemId == getShopType(TYPE)[i][0]) {
					...
					points = (SHOP == PK_POINTS ? player.getPKP() : SHOP == RUBY ? player.getRubyPoints() : player.getRessurgePoints());
					continue;
				}
		}
		if (canBuy) {
			if (amount * defaultPrice > (points != -1 ? points
					: player.getTotalCoins())) {
				amount = (points != -1 ? points : player.getTotalCoins())
						/ defaultPrice;
				price = defaultPrice * amount;
			} else
				price = defaultPrice * amount;
			if (amount < 1) {
				player.getPackets().sendGameMessage("You don't have enough " + getPointsName(TYPE, SHOP) + " to buy this.");
				return;
			}
