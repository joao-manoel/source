...
public final class Skills implements Serializable {
	...
	public void checkTrimedCape() {		
		if(howMany99() >= 2) {
			SkillCapes.checkTrimmeCape(player);
		}
	}
		...
		if (getXp(skill) >= 200000000) {
			...
			if ((skill >= 0 && skill <= 4) || skill == 6) {
				if (player.getTemporaryTarget() != null && player.isAtWild()
					...
					exp *= 1;
				else if (player.getTemporaryTarget() != null && player.isAtWild() && (player.getTemporaryTarget() instanceof NPC)) {
					
					//exp *= player.isLegendaryMember() ? (Settings.COMBAT_XP_RATE + 20) : player.isExtremeMember() ? (Settings.COMBAT_XP_RATE + 15) : player.isSuperMember() ? (Settings.COMBAT_XP_RATE + 10) :  player.isMember() ? (Settings.COMBAT_XP_RATE + 5) :  Settings.COMBAT_XP_RATE;
				}else {
					//exp *= player.isLegendaryMember() ? (Settings.COMBAT_XP_RATE + 20) : player.isExtremeMember() ? (Settings.COMBAT_XP_RATE + 15) : player.isSuperMember() ? (Settings.COMBAT_XP_RATE + 10) :  player.isMember() ? (Settings.COMBAT_XP_RATE + 5) :  Settings.COMBAT_XP_RATE;
				}
			} else if (skill == 23) {
				//exp *= player.isLegendaryMember() ? (Settings.SUMMONING_XP_RATE + 20) : player.isExtremeMember() ? (Settings.SUMMONING_XP_RATE + 15) : player.isSuperMember() ? (Settings.SUMMONING_XP_RATE + 10) :  player.isMember() ? (Settings.SUMMONING_XP_RATE + 5) :  Settings.SUMMONING_XP_RATE;
			} else if(skill == 24) {
				exp *= 1;
			} else if(skill == Skills.FARMING) {
				//exp *= player.isLegendaryMember() ? 40 : player.isExtremeMember() ? 35 : player.isSuperMember() ? 30  : player.isMember() ? 25 :  20;
			} else if (skill == Skills.PRAYER) {
				//exp *= player.isLegendaryMember() ? 28 : player.isExtremeMember() ? 23 : player.isSuperMember() ? 18 : player.isMember() ? 13 :  8;
			} else {
				//exp *= player.isLegendaryMember() ? (Settings.SKILLING_XP_RATE + 20) : player.isExtremeMember() ? (Settings.SKILLING_XP_RATE + 15) : player.isSuperMember() ? (Settings.SKILLING_XP_RATE + 10) :  player.isMember() ? (Settings.SKILLING_XP_RATE + 5) :  Settings.SKILLING_XP_RATE;
			}
			exp *= 1;
		}
