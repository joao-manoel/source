...
public class NPC extends Entity implements Serializable {
	...
	public void loadNPCSettings() {
		...
		if (getId() == 231 || getId() == 705 || getId() == 1861 || getId() == 4707 || getId() == 5195 || getId() == 4247 || getId() == 961) {
			setRandomWalk(0);
		}
		if (getId() == 650) {
		...
		if(getId() == 2253) {
			setName("Wise Old Man Teleports");
		}
	...
	public void sendDeath(final Entity source) {
		...
		setBonuses();
		// source.setAttackedByDelay(Utils.currentTimeMillis() +
		// (defs.getDeathDelay() * 1000));
		animate(null);
	...
	private void addAvalonPoints(Player killer, NPC npc, boolean wildy) {
		...
		killer.setRessurgePoints(killer.getRessurgePoints() + (int) (totalPoints + bonusPoints));
		killer.getPackets()
	...
	public enum BossIds {
		...
		DAGANNOTH_REX(2883), TORMENTED_DEMON_1(8349), TORMENTED_DEMON_2(8350), TORMENTED_DEMON_3(8351), CHAOS_ELEMENTAL(3200), NEX(13450);
	...
	public enum AchievementKills {
		...
		Commander_Zilyana(6247, 25), Nex(13450, 25);
	...
	public void setGodWarKill(Player player) {
		if(player.getControlerManager().getControler() instanceof GodWars) {
			GodWars godControler = (GodWars) player.getControlerManager().getControler();;
			switch (getId()) {
				case 6265:
				case 6261:
				case 6263:
				case 6260:
					...
					player.sm("Bandos: "+godControler.killCount[0]);
					break;
			}
		}
	}
	...
	public void getKillcount(Player player) {
		for (BossIds id : BossIds.values()) {
			if (getId() == id.getId()) {
				...
				player.getBossKillcount().put(getName().replace("_1", "").replace("_2",  "").replace("3_",  ""), totalKills);
				player.getPackets()
			...
			if (killer.dropTesting) {
				for (int i = 0; i < killer.dropTestingAmount; i++) {
					...
					if (droppedItems == null) {
						killer.getPackets().sendGameMessage(getName() + " has no drops/contact Support");
						return;
					} else {
	...
	public void loopDrojp() {
		...
		if (droppedItems == null) {
			...
			for (Item item : droppedItems) {
				killer.getBank().addItem(item, true);
			}
			return;
		}
	}

	public void sendNewDrop(Player player, NPC npc, Item item) {
		World.updateGroundItem(item, npc, player);
	...
	public void sendLootshare(Player player, Item item) {
		...
		if (fc != null) {
			...
			if (item.getAmount() > 0) {
				...
				luckyPlayer
						.sm(String.format(
								((isRareDrop(player)) ? "<col=ff0000>" : "<col=216902>") + "You received: %s x %s. ("
										+ getName() + ") </col>",
								Utils.getFormattedNumber(item.getAmount(), ','), dropName));
				for (Player p : playersWithLs) {
					if (!p.equals(luckyPlayer)) {
						p.sm(String.format("%s</col> received: %s x %s. (" + getName() + ") ", "<col=216902>" + luckyPlayer.getDisplayName(),
								Utils.getFormattedNumber(item.getAmount(), ','), dropName));
					}
				}
				...
				if (isRareDrop(player)) {
					sendLootBeam(item, luckyPlayer, this);
				} else if (isVeryRareDrop(luckyPlayer)) {
					sendLootBeam(item, luckyPlayer, this);
					...
					luckyPlayer.getAdventureLog()
							.addActivity("I found " + luckyPlayer.grammar(item) + " " + dropName + ".");
				}
				/*
				 * if (friendsDrop(dropName.toLowerCase())) { if
				 * (rareDrop(dropName.toLowerCase())) { return; } for (Player p2 :
				 * World.getPlayers()) { if
				 * (!p2.getDisplayName().equalsIgnoreCase(luckyPlayer.getDisplayName()) &&
				 * luckyPlayer.getFriendsIgnores().getFriends().contains(p2.getUsername()) &&
				 * p2.getFriendsIgnores().getFriends().contains(luckyPlayer.getUsername())) {
				 * p2.getPackets().sendGameMessage("<img=7><col=00b33c>News: " +
				 * luckyPlayer.getDisplayName() + " has recieved " + (item.getAmount() > 1 ?
				 * item.getAmount() + " x " + Utils.formatString(dropName) :
				 * Utils.formatString(dropName)) + " as loot from " + getName() + "!");
				 * luckyPlayer.getAdventureLog() .addActivity("I found " +
				 * luckyPlayer.grammar(item) + " " + dropName + "."); } } }
				 */
			}
		}
		sendDp = false;
	...
	public void sendDrop(Player player, Drop drop) {
		...
		if (Settings.DOUBLE_DROP && (drop.getRate() == 100 || item.getDefinitions().isStackable()
				|| item.getDefinitions().isNoted() || isRareDrop(player) || isVeryRareDrop(player)))
		...
		if (player.isToogleLootShare()) {
			...
			int valueableDrop = player.toggles("DROPVALUE", 0);
			Item i = item;
			if (item.getDefinitions().isNoted())
				i = new Item(item.getDefinitions().getCertId(), item.getAmount());
			if (Settings.DEBUG)
			System.out.println((i.getDefinitions().getTipitPrice() * i.getAmount()));
			if ((i.getDefinitions().getTipitPrice() * i.getAmount()) >= valueableDrop) {
				player.getPackets().sendGameMessage("<col=ff0000>Valuable drop: " + i.getName()
			...
			if (item.getAmount() > 0) {
				 ...
				 * (rareDrop(dropName.toLowerCase())) { return; } for (Player p2 :
				 * World.getPlayers()) { if
				 * (!p2.getDisplayName().equalsIgnoreCase(player.getDisplayName()) &&
				 * player.getFriendsIgnores().getFriends().contains(p2.getUsername()) &&
				 * p2.getFriendsIgnores().getFriends().contains(player.getUsername())) {
				 * p2.getPackets().sendGameMessage("<img=7><col=00b33c>News: " +
				 * player.getDisplayName() + " has recieved " + (item.getAmount() > 1 ?
				 * item.getAmount() + " x " + Utils.formatString(dropName) :
				 * Utils.formatString(dropName)) + " as loot from " + getName() + "!"); } } }
				 */
			}
	...
	private boolean isRareDrop(Player player) {
		if (player.isRareDrop())
			return true;
		return false;
	}

	private boolean isVeryRareDrop(Player player) {
		if (player.isVeryRareDrop())
			return true;
		return false;
	}

	public void sendLootBeam(Item item, Player player, NPC npc) {
		player.setVeryRareDrop(false);
		player.setRareDrop(false);
		if (!player.toggles("LOOTBEAMS", false)) {
			return;
		}
		int x = getCoordFaceX(getSize());
		int y = getCoordFaceY(getSize());
		int z = getPlane();
		player.getPackets().sendGameMessage("<col=b25200>A loot beam appears on your "
				+ (player.isVeryRareDrop() ? "very rare" : "rare") + " drop.");
		World.sendPrivateGraphics(player, new Graphics(7, 0, 0), new WorldTile(x, y, z));
																	...
																	|| (!forceMultiAttacked
																			&& (!isAtMultiArea() || !player
																					...
																					&& player.getAttackedBy() != this && (player
																							...
																							|| (!forceAgressive && !WildernessControler.isAtWild(this) && player
