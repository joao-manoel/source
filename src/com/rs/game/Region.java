...
public class Region {
	...
	public void removeObject(WorldObject object, int plane, int localX, int localY) {
		Logger.log(this, "id: " +object.getId());
		if (objects == null)
			objects = new WorldObject[4][64][64][4];
		int slot = OBJECT_SLOTS[object.getType()];
		WorldObject removed = getRemovedObjectWithSlot(plane, localX, localY, slot);
	...
	public WorldObject getObjectWithSlot(int plane, int x, int y, int slot) {
		...
		if (o == null) {
			if (getRemovedObjectWithSlot(plane, x, y, slot) != null) {
				System.out.println("removeobjectwithslot null");
				return null;
			}
	...
	public void handleDeletedObjects() {
		for (int x = 0; x < 10; x++) {// House north of edge bank
			for (int y = 0; y < 7; y++) {
				World.deleteObject(new WorldTile(3091 + x, 3507 + y, 0));
			}
		}
		for (int x = 0; x < 2; x++) {// Wall, house north of edge bank
			for (int y = 0; y < 3; y++) {
				World.deleteObject(new WorldTile(3092 + x, 3513 + y, 0));
			}
		}
