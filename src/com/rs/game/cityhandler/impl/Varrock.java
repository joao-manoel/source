...
public class Varrock implements CityEvent {
	...
	public boolean init() {
		...
		registerObject(9312, this);
		registerObject(9311, this);
		registerObject(28094, this);
	...
	public boolean handleNPCClick4(Player player, NPC npc) {
		...
		case 5913:
			...
			OldMagicSystem.sendNormalTeleportSpell(player, 0, 0, new WorldTile(2909, 4834, 0));
			break;
		}
		return false;
	}
	...
	public boolean handleObjectClick(Player player, WorldObject object) {
		...
		if(id == 9311) {
			...
			if(player.getX() >= 3127 && player.getX() <= 3138
					&& player.getY() >= 3506 && player.getY() <= 3520) {// fora > dentro
		    	UnderwallTunnel(player, new WorldTile(3143,3514,0), new WorldTile(3144,3514,0));
		    	return true;
		    }
		}
		if (id == 9312) {
		    ...
		    if(player.getX() >= 3141 && player.getX() <= 3151
					&& player.getY() >= 3503 && player.getY() <= 3515) { //dentro > fora
		    	UnderwallTunnel(player, new WorldTile(3139, 3516, 0),  new WorldTile(3138, 3516, 0));
		    	return true;
		    }
		    
		}
		if(id == 28094) {
			player.getDialogueManager().startDialogue("KeldagrimTravel");
			return true;
		}
		return false;
	}
	...
	public void UnderwallTunnel(Player player, WorldTile WorldTile1, WorldTile WorldTile2) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				player.setNextWorldTile(WorldTile1);
			}
		}, 1);
		...
		WorldTasksManager.schedule(new WorldTask() {
			@Override
			public void run() {
				player.setNextWorldTile(WorldTile2);
			}
		}, 1);
