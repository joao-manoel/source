...
public final class Hobgoblin extends MobRewardNodeBuilder {
	...
	public void populate(Player player) {
		...
		case -1:
		case COMMON:
			dissectNodeBatch(1, node(BRONZE_SPEAR, 1), 
					node(IRON_JAVELIN, 5), 
					node(IRON_SWORD, 1), 
			...
			break;
		case UNCOMMON:
			dissectNodeBatch(1, node(IRON_LONGSWORD, 1), 
					node(IRON_SPEAR, 1), 
					node(STEEL_SPEAR, 1), 
			...
			break;
		case RARE:
			dissectNodeBatch(1, node(CADANTINE_SEED, 1), 
			...
			break;
		case VERY_RARE:
			dissectNodeBatch(1, node(TORSTOL_SEED, 1));
			break;
		}
		addObj(BONES, 1);
